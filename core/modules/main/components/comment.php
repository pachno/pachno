                ...
                <?php if ((\pachno\core\framework\Context::isProjectContext() && !\pachno\core\framework\Context::getCurrentProject()->isArchived()) || !\pachno\core\framework\Context::isProjectContext()) : ?>
                    <div class="tools action-buttons">
                        ...
                        <?php if ($comment->canUserDelete($pachno_user)): ?>
                            <a href="javascript:void(0);" class="action-button" onclick="Pachno.UI.Dialog.show('<?php echo __('Do you really want to delete this comment?'); ?>', '<?php echo __('Please confirm that you want to delete this comment.'); ?>', {yes: {click: function() {Pachno.trigger(Pachno.EVENTS.comment.remove, { url: '<?= make_url('comment_delete', ['comment_applies_id' => $comment->getTargetID(), 'comment_applies_type' => $comment->getTargetType(), 'comment_module' => $comment->getModuleName(), 'comment_id' => $comment->getID()]); ?>', type: 'comment', id: <?= $comment->getID(); ?> })}}, no: {click: Pachno.UI.Dialog.dismiss}});"><?php echo fa_image_tag('trash-alt', [], 'far'); ?></a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
