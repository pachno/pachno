<li id="viewissue_links_<?php echo $link_id; ?>" class="attached_item">
    ...
    <?php if ($issue->canRemoveAttachments()): ?>
        <div class="removelink">
            <?php echo javascript_link_tag(image_tag('action_delete.png'), array('id' => $link_id . '_remove_link', 'onclick' => "Pachno.UI.Dialog.show('".__('Do you really want to remove this link?')."', '".__('This action cannot be reversed. Are you sure you want to do this?')."', {yes: {click: function() {Pachno.Issues.Link.remove('".make_url('issue_remove_link', array('issue_id' => $issue->getID(), 'link_id' => $link_id))."', ".$link_id."); }}, no: { click: Pachno.UI.Dialog.dismiss }});")); ?>
