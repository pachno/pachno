        ...
        <div class="td">
            ...
            <div class="date">
                <?php if ($milestone->getStartingDate()): ?>
                    <?php
                        echo \pachno\core\framework\Context::getI18n()->formatTime($milestone->getStartingDate(), 23, true, true) . ' - ';
                        if ($milestone->getScheduledDate() > 0): echo \pachno\core\framework\Context::getI18n()->formatTime($milestone->getScheduledDate(), 23, true, true); else: echo __('No scheduled date specified'); endif;
                    ?>
                <?php else: ?>
                    <?php if ($milestone->getScheduledDate() > 0): echo __('Scheduled for %scheduled_date', array('%scheduled_date' => \pachno\core\framework\Context::getI18n()->formatTime($milestone->getScheduledDate(), 23, true, true))); else: echo __('No scheduled date specified'); endif; ?>
                <?php endif; ?>
            </div>

            <?php if ($milestone->isReached()): ?>
                <div class="status">
                    <?php if ($milestone->getType() == \pachno\core\entities\Milestone::TYPE_REGULAR): ?>
