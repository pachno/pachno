    ...
    try {
        ...
        switch ($item->getChangeType()) {
            ...
            case LogItem::ACTION_ISSUE_UPDATE_STATUS:
                $new_item = ListTypes::getTable()->selectById((int) $item->getCurrentValue());
                ...
                if ($item->hasChangeDetails()) {
                    $previous_value = ($item->getPreviousValue()) ? (($old_item = ListTypes::getTable()->selectById((int) $item->getPreviousValue())) ? __($old_item->getName()) : __('Unknown')) : __('Not determined');
                    $new_value = ($item->getCurrentValue()) ? (($new_item = ListTypes::getTable()->selectById((int) $item->getCurrentValue())) ? __($new_item->getName()) : __('Unknown')) : __('Not determined');
            ...
            case LogItem::ACTION_ISSUE_UPDATE_MILESTONE:
                ...
                if ($item->hasChangeDetails()) {
                    $previous_value = ($item->getPreviousValue()) ? (($old_item = \pachno\core\entities\tables\Milestones::getTable()->selectById((int) $item->getPreviousValue())) ? __($old_item->getName()) : __('Unknown')) : __('Not determined');
                    $new_value = ($item->getCurrentValue()) ? (($new_item = \pachno\core\entities\tables\Milestones::getTable()->selectById((int) $item->getCurrentValue())) ? __($new_item->getName()) : __('Unknown')) : __('Not determined');
            ...
            case LogItem::ACTION_ISSUE_UPDATE_POSTED_BY:
                ...
                if ($item->hasChangeDetails()) {
                    $previous_value = ($item->getPreviousValue()) ? (($old_item = \pachno\core\entities\tables\Users::getTable()->selectById((int) $item->getPreviousValue())) ? get_component_html('main/userdropdown_inline', ['user' => $old_item]) : '<strong>' . __('Unknown') . '</strong>') : '<strong>' . __('Not determined') . '</strong>';
                    $new_value = ($item->getCurrentValue()) ? (($new_item = \pachno\core\entities\tables\Users::getTable()->selectById((int) $item->getCurrentValue())) ? get_component_html('main/userdropdown_inline', ['user' => $new_item]) : '<strong>' . __('Unknown') . '</strong>') : '<strong>' . __('Not determined') . '</strong>';
                        ...
                        switch ($customdatatype->getType()) {
                            ...
                            case DatatypeBase::STATUS_CHOICE:
                                ...
                                try {
                                    switch ($customdatatype->getType()) {
                                        case DatatypeBase::EDITIONS_CHOICE:
                                            $old_object = \pachno\core\entities\tables\Editions::getTable()->selectById((int) $old_value);
                                            break;
                                        case DatatypeBase::COMPONENTS_CHOICE:
                                            $old_object = \pachno\core\entities\tables\Components::getTable()->selectById((int) $old_value);
                                            break;
                                        case DatatypeBase::RELEASES_CHOICE:
                                            $old_object = \pachno\core\entities\tables\Builds::getTable()->selectById((int) $old_value);
                                            break;
                                        case DatatypeBase::STATUS_CHOICE:
                                            $old_object = ListTypes::getTable()->selectById((int) $old_value);
                                            break;
                                    }
                                } catch (\Exception $e) {
                                }
                                try {
                                    switch ($customdatatype->getType()) {
                                        case DatatypeBase::EDITIONS_CHOICE:
                                            $new_object = \pachno\core\entities\tables\Editions::getTable()->selectById((int) $new_value);
                                            break;
                                        case DatatypeBase::COMPONENTS_CHOICE:
                                            $new_object = \pachno\core\entities\tables\Components::getTable()->selectById((int) $new_value);
                                            break;
                                        case DatatypeBase::RELEASES_CHOICE:
                                            $new_object = \pachno\core\entities\tables\Builds::getTable()->selectById((int) $new_value);
                                            break;
                                        case DatatypeBase::STATUS_CHOICE:
                                            $new_object = ListTypes::getTable()->selectById((int) $new_value);
                                            break;
                                    }
                                } catch (\Exception $e) {
                                }
