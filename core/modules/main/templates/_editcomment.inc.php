<div id="comment_edit_<?= $comment->getID(); ?>" class="comment_edit comment_editor editor_container">
    <form id="comment_edit_form_<?= $comment->getID(); ?>" class="syntax_<?= \pachno\core\framework\Settings::getSyntaxClass($comment->getSyntax()); ?>" action="<?= make_url('comment_update', array('comment_id' => $comment->getID())); ?>" method="post" onSubmit="Pachno.Main.Comment.update('<?= make_url('comment_update', array('comment_id' => $comment->getID())); ?>', '<?= $comment->getID(); ?>'); return false;">
        ...
        <?php include_component('main/textarea', array('area_name' => 'comment_body', 'target_type' => isset($mentionable_target_type) ? $mentionable_target_type : $comment->getTargetType(), 'target_id' => $comment->getTargetId(), 'area_id' => 'comment_edit_'.$comment->getID().'_bodybox', 'height' => '200px', 'width' => '100%', 'syntax' => \pachno\core\framework\Settings::getSyntaxClass($comment->getSyntax()), 'value' => \pachno\core\framework\Context::getI18n()->decodeUTF8($comment->getContent(), true))); ?>
        <div id="comment_edit_controls_<?= $comment->getID(); ?>" class="backdrop_details_submit">
            ...
            <div class="submit_container"><button type="submit" class="button"><?= image_tag('spinning_16.gif', ['id' => 'comment_edit_indicator_' . $comment->getID(), 'style' => 'display: none;']) . __('Save changes'); ?></button></div>
        </div>
    </form>
</div>
