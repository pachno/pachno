...
<?php if ($item->getTargetType() == LogItem::TYPE_COMMIT && $item->getCommit() instanceof Commit): ?>
    <tr>
        ...
        <td style="clear: both;<?php if (!isset($include_issue_title) || $include_issue_title): ?> padding-bottom: <?php echo (isset($extra_padding) && $extra_padding) ? 15 : 10; ?>px;<?php endif; ?>">
            <?php if ((!isset($include_issue_title) || $include_issue_title) && (isset($include_time) && $include_time == true)): ?><span class="time"><?php echo \pachno\core\framework\Context::getI18n()->formatTime($item->getTime(), 19); ?></span>&nbsp;<?php endif; ?>
            <?php if (!isset($include_issue_title) || $include_issue_title): ?>
    ...
    <tr>
        ...
        <td style="clear: both;<?php if (!isset($include_issue_title) || $include_issue_title): ?> padding-bottom: <?php echo (isset($extra_padding) && $extra_padding) ? 15 : 10; ?>px;<?php endif; ?>">
            ...
            <?php 
                ...
                $title = \pachno\core\framework\Context::getI18n()->decodeUTF8($item->getIssue()->getFormattedTitle(true));
                if (isset($pad_length))
                {
                    $title = pachno_truncateText($title, $pad_length);
                }
