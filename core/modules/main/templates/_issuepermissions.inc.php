<div class="backdrop_box medium issue_access_policy_box" id="viewissue_add_item_div">
    ...
    <div id="backdrop_detail_content" class="backdrop_detail_content">
        ...
        <input type="radio" name="issue_access" id="issue_access_public_category" onchange="Pachno.Issues.ACL.toggle_checkboxes(this, <?php echo $issue->getID(); ?>, 'public_category');" value="public_category"<?php if($issue->isUnlocked() && $issue->isLockedCategory()): ?> checked<?php endif; ?>><label for="issue_access_public_category"><?php echo __('Available to anyone with access to project, category and those listed below'); ?></label><br>
        <input type="radio" name="issue_access" id="issue_access_restricted" onchange="Pachno.Issues.ACL.toggle_checkboxes(this, <?php echo $issue->getID(); ?>, 'restricted');" value="restricted"<?php if($issue->isLocked()): ?> checked<?php endif; ?>><label for="issue_access_restricted"><?php echo __('Available only to you and those listed below'); ?></label><br>
        ...
        <div id="acl-users-teams-selector" style="<?php if($issue->isUnlocked() && $issue->isUnlockedCategory()): ?> display: none;<?php endif; ?>">
            ...
            <?php include_component('main/identifiableselector', array(    'html_id'             => "popup_find_acl_{$issue->getID()}",
                                                                      ...
                                                                      'callback'             => "Pachno.Issues.ACL.addTarget('" . make_url('getacl_formentry', array('identifiable_type' => 'user', 'identifiable_value' => '%identifiable_value')) . "', {$issue->getID()});",
                                                                      'team_callback'     => "Pachno.Issues.ACL.addTarget('" . make_url('getacl_formentry', array('identifiable_type' => 'team', 'identifiable_value' => '%identifiable_value')) . "', {$issue->getID()});",
