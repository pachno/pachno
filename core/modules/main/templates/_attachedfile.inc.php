    ...
    <li id="<?php echo $base_id . '_' . $file_id; ?>" class="attached_item <?php if ($file->isImage()) echo 'file_image'; ?>">
        <?php if ($file->isImage()): ?>
            ...
            <div class="embedlink removelink">
                <?php echo javascript_link_tag(image_tag('action_embed.png'), array('onclick' => "Pachno.UI.Dialog.showModal('".__('Embedding this file in descriptions or comments')."', '".__('Use this tag to include this image: [[Image:%filename|thumb|Image description]]', array('%filename' => $file->getRealFilename()))."');")); ?>
            </div>
        <?php else: ?>
        ...
        <?php if ($can_remove): ?>
            <div class="removelink">
                <?php if ($mode == 'issue'): ?>
                    <?php echo javascript_link_tag(fa_image_tag('times', ['class' => 'delete']), array('id' => $base_id . '_' . $file_id . '_remove_link', 'onclick' => "Pachno.UI.Dialog.show('".__('Do you really want to detach this file?')."', '".__('If you detach this file, it will be deleted. This action cannot be undone. Are you sure you want to remove this file?')."', {yes: {click: function() {Pachno.Issues.File.remove('".make_url('issue_detach_file', array('issue_id' => $issue->getID(), 'file_id' => $file_id))."', ".$file_id."); }}, no: { click: Pachno.UI.Dialog.dismiss }});")); ?>
                <?php elseif ($mode == 'article'): ?>
                    <?php echo javascript_link_tag(fa_image_tag('times', ['class' => 'delete']), array('id' => $base_id . '_' . $file_id . '_remove_link', 'onclick' => "Pachno.UI.Dialog.show('".__('Do you really want to detach this file?')."', '".__('If you detach this file, it will be deleted. This action cannot be undone. Are you sure you want to remove this file?')."', {yes: {click: function() {Pachno.Main.detachFileFromArticle('".make_url('article_detach_file', array('article_name' => $article->getName(), 'file_id' => $file_id))."', ".$file_id.", ".$article->getID()."); }}, no: { click: Pachno.UI.Dialog.dismiss }});")); ?>
                <?php endif; ?>
                <?php echo image_tag('spinning_16.gif', array('id' => $base_id . '_' . $file_id . '_remove_indicator', 'style' => 'display: none;')); ?>
            </div>
        <?php endif; ?>
        <div class="upload_details">
            <?php echo __('%filename uploaded %date by %username', array('%filename' => '<span class="filename">'.$file->getOriginalFilename().'</span>', '%date' => \pachno\core\framework\Context::getI18n()->formatTime($file->getUploadedAt(), 23), '%username' => (($file->getUploadedBy() instanceof \pachno\core\entities\User) ? '<a href="javascript:void(0);" onclick="Pachno.UI.Backdrop.show(\'' . make_url('get_partial_for_backdrop', array('key' => 'usercard', 'user_id' => $file->getUploadedBy()->getID())) . '\');" class="faded_out">' . $file->getUploadedBy()->getNameWithUsername() . '</a>' : __('unknown user')))); ?>
        </div>
    </li>
<?php else: ?>
