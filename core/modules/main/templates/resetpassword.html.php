    ...
    <form accept-charset="<?= \pachno\core\framework\Context::getI18n()->getCharset(); ?>" action="<?= make_url('reset_password', array('user' => $user->getUsername(), 'reset_hash' => $user->getActivationKey())); ?>" method="post" onsubmit="['reset_button', 'reset_indicator'].each(Element.toggle);">
        ...
        <div class="backdrop_detail_content login_content">
            <div class="logindiv regular active" id="reset_password_container">
                <input type="hidden" id="pachno_referer" name="reset_referer" value="<?= make_url('home'); ?>" />
                <ul class="login_formlist">
                    <li>
                        <label for="pachno_username"><?= __('Username'); ?></label>
                        <input type="text" id="pachno_username" name="username" value="<?= $user->getUsername(); ?>" disabled>
                    </li>
                    <li>
        ...
        <div class="backdrop_details_submit">
            ...
            <div class="submit_container">
                <button type="submit" id="reset_button" class="button"><?= image_tag('spinning_16.gif', array('id' => 'reset_indicator', 'style' => 'display: none;')) . __('Reset my password'); ?></button>
            </div>
        </div>
    </form>
</div>
