        ...
        <?php if (!$issue->getProject()->isArchived() && $issue->canEditIssueDetails()): ?>
                    ...
                    <?php foreach ($issue->getAvailableWorkflowTransitions() as $transition): ?>
                        <li>
                            <?php if ($transition->hasTemplate()): ?>
                                <?php echo javascript_link_tag($transition->getName(), array('onclick' => "Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'workflow_transition', 'transition_id' => $transition->getID()))."&project_key=".$issue->getProject()->getKey()."&issue_id=".$issue->getID()."');")); ?>
                            <?php else: ?>
                                <?php echo javascript_link_tag(image_tag('spinning_16.gif', array('style' => 'display: none;', 'id' => 'transition_working_'.$transition->getID().'_indicator')).$transition->getName(), array('onclick' => "Pachno.Search.interactiveWorkflowTransition('".make_url('transition_issues', array('project_key' => $issue->getProject()->getKey(), 'transition_id' => $transition->getID()))."&issue_ids[]=".$issue->getID()."', ".$transition->getID().");")); ?>
                            <?php endif; ?>
                        </li>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <?php if ((!isset($multi) || !$multi) && $issue->isUpdateable() && $issue->canAttachLinks()): ?>
                <?php if ($issue->canAttachLinks()): ?>
                    <li><a href="javascript:void(0);" id="attach_link_button" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'attachlink', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('link').__('Attach a link'); ?></a></li>
                <?php endif; ?>
            <?php endif; ?>
            <?php if ($issue->isUpdateable() && \pachno\core\framework\Settings::isUploadsEnabled() && $issue->canAttachFiles()): ?>
                <?php if (\pachno\core\framework\Settings::isUploadsEnabled() && $issue->canAttachFiles()): ?>
                    <li><a href="javascript:void(0);" id="attach_file_button" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.showUploader('<?php echo make_url('get_partial_for_backdrop', array('key' => 'uploader', 'mode' => 'issue', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('paperclip').__('Attach a file'); ?></a></li>
                <?php else: ?>
                    <li class="disabled"><a href="javascript:void(0);" id="attach_file_button" onclick="Pachno.Main.Helpers.Message.error('<?php echo __('File uploads are not enabled'); ?>', '<?php echo __('Before you can upload attachments, file uploads needs to be activated'); ?>');"><?php echo fa_image_tag('paperclip').__('Attach a file'); ?></a></li>
                <?php endif; ?>
            <?php endif; ?>
            <?php if ($issue->isUpdateable()): ?>
                <?php if ($issue->canEditAffectedComponents() || $issue->canEditAffectedBuilds() || $issue->canEditAffectedEditions()): ?>
                    <li><a id="affected_add_button" href="javascript:void(0);" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issue_add_item', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('cubes', ['class' => 'affected_items']).__('Add affected item'); ?></a></li>
                <?php else: ?>
                    <li class="disabled"><a id="affected_add_button" href="javascript:void(0);" onclick="Pachno.Main.Helpers.Message.error('<?php echo __('You are not allowed to add an item to this list'); ?>');"><?php echo fa_image_tag('cubes', ['class' => 'affected_items']).__('Add affected item'); ?></a></li>
                <?php endif; ?>
                ...
                <?php if ($issue->canAddRelatedIssues() && $pachno_user->canReportIssues($issue->getProject())): ?>
                    <?php if (isset($board)): ?>
                        <?php if (!$board->getTaskIssuetypeID()): ?>
                            <li><?php echo javascript_link_tag(fa_image_tag('list-alt').__('Create a new child issue'), array('onclick' => "Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'reportissue', 'project_id' => $issue->getProject()->getId(), 'parent_issue_id' => $issue->getID()))."');", 'title' => __('Create a new child issue'))); ?></li>
                        <?php elseif ($issue->getIssuetype()->getID() != $board->getTaskIssuetypeID()): ?>
                            <li><?php echo javascript_link_tag(fa_image_tag('list-alt').__('Add a new task'), array('onclick' => "Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'reportissue', 'project_id' => $issue->getProject()->getId(), 'parent_issue_id' => $issue->getID(), 'issuetype_id' => $board->getTaskIssuetypeID(), 'lock_issuetype' => 1))."');", 'title' => __('Add a new task'))); ?></li>
                        <?php endif; ?>
                    <?php else: ?>
                        <li><?php echo javascript_link_tag(fa_image_tag('list-alt').__('Create a new related issue'), array('onclick' => "Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'reportissue', 'project_id' => $issue->getProject()->getId(), 'parent_issue_id' => $issue->getID()))."');", 'title' => __('Create a new child issue'))); ?></li>
                    <?php endif; ?>
                <?php endif; ?>
                <?php if ($issue->canAddRelatedIssues()): ?>
                    <li><a href="javascript:void(0)" id="relate_to_existing_issue_button" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'relate_issue', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('share-alt').__('Add a relation for this issue'); ?></a></li>
                <?php endif; ?>
            <?php else: ?>
                <?php if ($issue->canAddRelatedIssues() && $pachno_user->canReportIssues($issue->getProject())): ?>
            ...
            <?php if ($issue->canEditSpentTime()): ?>
                <li><a href="javascript:void(0)" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issue_spenttimes', 'issue_id' => $issue->getID(), 'initial_view' => 'entry')); ?>');"><?php echo fa_image_tag('clock').__('Log time spent'); ?></a></li>
            <?php endif; ?>
            <?php if ($issue->canEditAccessPolicy()): ?>
                <li class="separator"></li>
                <li><a href="javascript:void(0)" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issue_permissions', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('lock', ['class' => 'access_policy']).__("Update issue access policy"); ?></a></li>
                <li><a href="javascript:void(0)" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issue_subscribers', 'issue_id' => $issue->getID())); ?>');"><?php echo fa_image_tag('star', ['class' => 'subscriber_list']).__("Manage issue subscribers"); ?></a></li>
            <?php endif; ?>
            <?php if ($issue->canEditIssueDetails()): ?>
                <li><a href="javascript:void(0)" onclick="Pachno.Main.Profile.clearPopupsAndButtons();Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'move_issue', 'issue_id' => $issue->getID(), 'multi' => (int) (isset($multi) && $multi))); ?>');"><?php echo fa_image_tag('exchange-alt').__("Move issue to another project"); ?></a></li>
            <?php endif; ?>
