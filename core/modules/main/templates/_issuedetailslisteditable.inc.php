            ...
            <dl class="viewissue_list">
                ...
                <dd class="hoverable">
                    ...
                    <div style="display: inline-block; float: left; width: auto;<?php if (!$issue->getStatus() instanceof \pachno\core\entities\Datatype): ?> display: none;<?php endif; ?>" id="status_table">
                        <div class="status_badge" style="background-color: <?php echo ($issue->getStatus() instanceof \pachno\core\entities\Datatype) ? $issue->getStatus()->getColor() : '#FFF'; ?>;color: <?php echo ($issue->getStatus() instanceof \pachno\core\entities\Datatype) ? $issue->getStatus()->getTextColor() : '#333'; ?>;" id="status_<?php echo $issue->getID(); ?>_color">
                                                    ...
                                                    <?php foreach (\pachno\core\entities\Status::getAll() as $choice): ?>
                                                        <li>
                                                            <a href="javascript:void(0);" onclick="Pachno.Issues.Field.set('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => $field, $field . '_value' => $choice->getID())); ?>', '<?php echo $field; ?>');">
                                                                <div class="status_badge" style="background-color: <?php echo ($choice instanceof \pachno\core\entities\Status) ? $choice->getColor() : '#FFF'; ?>;">
