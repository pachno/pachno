<?php if (isset($target)): ?>
<form action="<?= make_url('update_attachments', array('target' => $mode, 'target_id' => $target->getID())); ?>" method="post" onsubmit="Pachno.Main.updateAttachments(this);return false;">
...
<?php endif; ?>
        ...
        <div class="upload_file_listing">
            <ul id="file_upload_list" data-filename-label="<?= htmlentities(__('File'), ENT_COMPAT, \pachno\core\framework\Context::getI18n()->getCharset()); ?>" data-description-label="<?= htmlentities(__('Description'), ENT_COMPAT, \pachno\core\framework\Context::getI18n()->getCharset()); ?>" data-description-placeholder="<?= htmlentities(__('Enter a short file description here'), ENT_COMPAT, \pachno\core\framework\Context::getI18n()->getCharset()); ?>" data-preview-src="<?= image_url('icon_file_generic.png'); ?>">
            ...
            require(['domReady', 'pachno/index'], function (domReady, Pachno) {
                domReady(function () {
                    ...
                    if ('ondrop' in document.createElement('span')) {
                        upload_container.addEventListener('dragover', Pachno.Main.dragOverFiles, false);
                        upload_container.addEventListener('dragleave', Pachno.Main.dragOverFiles, false);
                        upload_container.addEventListener('drop', Pachno.Main.dropFiles, false);
