...
<div id="backdrop_detail_content" class="backdrop_detail_content">
    ...
    <div style="text-align: right;">
        <a href="javascript:void(0)" class="button" onclick="if (jQuery('.milestone_details_link.selected').eq(0).find('> a:first-child').length) {jQuery('.milestone_details_link.selected').eq(0).find('> a:first-child').trigger('click');} else {Pachno.Main.Helpers.Backdrop.reset();}"><?php echo __('Got it'); ?></a>
    </div>
</div>
