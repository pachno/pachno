...
class Main extends framework\Action
{
     ...
     * - Issue ID.
     *
     * @param \pachno\core\entities\Issue $a Issue to compare.
     * @param \pachno\core\entities\Issue $b Issue to compare.
     *
            ...
            else
            {
                switch ($request['say'])
                {
                    ...
                    case 'getsearchcounts':
                        ...
                        foreach ($request['search_ids'] as $search_id) {
                            if (is_numeric($search_id)) {
                                ...
                                $search = \pachno\core\entities\SavedSearch::getPredefinedSearchObject($predefined_id);
                            }
                    ...
                    case 'get_mentionables':
                        ...
                        $mentionables = array();
                        if (isset($target) && $target instanceof \pachno\core\helpers\MentionableProvider)
                        {
    ...
    public function runMyAccount(framework\Request $request)
    {
        ...
        $categories = \pachno\core\entities\Category::getAll();
        $projects = [];
        $project_subscription_key = \pachno\core\framework\Settings::SETTINGS_USER_SUBSCRIBE_NEW_ISSUES_MY_PROJECTS;
        $category_subscription_key = \pachno\core\framework\Settings::SETTINGS_USER_SUBSCRIBE_NEW_ISSUES_MY_PROJECTS_CATEGORY;
        $category_notification_key = \pachno\core\framework\Settings::SETTINGS_USER_NOTIFY_NEW_ISSUES_MY_PROJECTS_CATEGORY;
        ...
        foreach (\pachno\core\entities\Project::getAll() as $project_id => $project) {
    ...
    protected function _postIssue(framework\Request $request)
    {
        ...
        framework\Event::listen('core', 'pachno\core\entities\Issue::createNew_pre_notifications', array($this, 'listen_issueCreate'));
        $issue->save();
    ...
    public function runIssueEditTimeSpent(framework\Request $request)
    {
        ...
        framework\Context::loadLibrary('common');
        $spenttime->editOrAdd($issue, $this->getUser(), array_only_with_default($request->getParameters(), array_merge(array('timespent_manual', 'timespent_specified_type', 'timespent_specified_value', 'timespent_activitytype', 'timespent_comment', 'edited_at'), \pachno\core\entities\common\Timeable::getUnitsWithPoints())));
    ...
    public function runUnlockIssue(framework\Request $request)
    {
        if ($issue_id = $request['issue_id'])
        {
            ...
            framework\Event::listen('core', 'pachno\core\entities\Issue::save_pre_notifications', array($this, 'listen_issueSaveUnlock'));
            $issue->setLocked(false);
            $issue->setLockedCategory($request->hasParameter('public_category'));
            $issue->save();
        }
    ...
    public function runLockIssue(framework\Request $request)
    {
        if ($issue_id = $request['issue_id'])
        {
            ...
            framework\Event::listen('core', 'pachno\core\entities\Issue::save_pre_notifications', array($this, 'listen_issueSaveLock'));
            $issue->setLocked();
            $issue->save();
        }
        else
    ...
    public function runGetFile(framework\Request $request)
    {
        ...
        if ($file instanceof entities\File)
        {
            if ($file->hasAccess())
            {
                ...
                if ($file->isImage() && \pachno\core\framework\Settings::isUploadsImageCachingEnabled()) {
                ...
                if (is_resource($fh))
                {
                    if ($isFile && \pachno\core\framework\Settings::isUploadsDeliveryUseXsend()) {
    ...
    public function runEditComment(framework\Request $request)
    {
        ...
        return $this->renderJSON(array('title' => $i18n->__('Comment added!'), 'comment_data' => $comment_html, 'comment_id' => $comment->getID(), 'commentcount' => entities\Comment::countComments($request['comment_applies_id'], $request['comment_applies_type']/* , $request['comment_module'] */)));
    }
