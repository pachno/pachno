...
<div data-workflow-transition data-id="<?php echo $transition->getID(); ?>" class="configurable-component workflow-transition form-container">
    <form class="row" accept-charset="<?php echo \pachno\core\framework\Context::getI18n()->getCharset(); ?>" action="<?php echo make_url('configure_workflow_transition_post', ['workflow_id' => $transition->getWorkflow()->getID(), 'transition_id' => $transition->getID()]); ?>" onsubmit="Pachno.Config.Workflows.Transition.save(this);return false;" data-interactive-form>
        ...
        <div class="icon">
            <button class="button secondary" onclick="Pachno.UI.Backdrop.show('<?= make_url('get_partial_for_backdrop', ['key' => 'edit_workflow_transition', 'transition_id' => $transition->getId(), 'step_id' => $step->getId()]); ?>');return false;">
                <?= fa_image_tag('edit', ['class' => 'icon'], 'far'); ?>
            </button>
            <a href="javascript:void(0);" class="button secondary icon" onclick="Pachno.UI.Dialog.show('<?php echo __('Really delete this transition?'); ?>', '<?php echo __('Are you really sure you want to delete this transition?'); ?>', {yes: {click: function() {Pachno.Config.Issuefields.Options.remove('<?php echo make_url('configure_workflow_transition_delete', ['workflow_id' => $transition->getWorkflow()->getID(), 'transition_id' => $transition->getID()]); ?>'); }}, no: {click: Pachno.UI.Dialog.dismiss}});"><?php echo fa_image_tag('trash-alt', [], 'far'); ?></a>
        </div>
            <?php /*
        <div id="item_option_<?php echo $type; ?>_<?php echo $transition->getID(); ?>_content">
            ...
            <div class="button-group" style="float: right;">
                ...
                <a href="javascript:void(0);" class="button button-icon" onclick="Pachno.UI.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issuefield_permissions', 'item_id' => $transition->getID(), 'item_key' => $transition->getPermissionsKey(), 'access_level' => $access_level)); ?>');" title="<?php echo __('Set permissions for this item'); ?>"><?php echo fa_image_tag('user-lock'); ?></a>
                <?php if ($transition->canBeDeleted()): ?>
                    <a href="javascript:void(0);" class="button button-icon" onclick="Pachno.UI.Dialog.show('<?php echo __('Really delete %itemname?', array('%itemname' => $transition->getName())); ?>', '<?php echo __('Are you really sure you want to delete this item?'); ?>', {yes: {click: function() {Pachno.Config.Issuefields.Options.remove('<?php echo make_url('configure_issuefields_delete', array('type' => $type, 'id' => $transition->getID())); ?>', '<?php echo $type; ?>', <?php echo $transition->getID(); ?>); Pachno.UI.Dialog.dismiss(); }}, no: {click: Pachno.UI.Dialog.dismiss}});" id="delete_<?php echo $transition->getID(); ?>_link"><?php echo fa_image_tag('trash-alt', [], 'far'); ?></a>
                <?php else: ?>
                    <a href="javascript:void(0);" class="button button-icon disabled" onclick="Pachno.UI.Message.error('<?php echo __('This item cannot be deleted'); ?>', '<?php echo __('Other items - such as workflow steps - may depend on this item to exist. Remove the dependant item or unlink it from this item to continue.'); ?>');" id="delete_<?php echo $transition->getID(); ?>_link"><?php echo fa_image_tag('trash-alt', [], 'far'); ?></a>
                <?php endif; ?>
            </div>
