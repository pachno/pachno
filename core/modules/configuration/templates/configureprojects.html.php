    ...
    <div>
        ...
        <div class="content faded_out">
            <p>
                ...
                <?php if (\pachno\core\framework\Context::getScope()->getMaxProjects()): ?>
                    <div class="faded_out dark" style="margin: 12px 0;">
                        <?php echo __('This instance is using %num of max %max projects', array('%num' => '<b id="current_project_num_count">'.\pachno\core\entities\Project::getProjectsCount().'</b>', '%max' => '<b>'.\pachno\core\framework\Context::getScope()->getMaxProjects().'</b>')); ?>
                    </div>
                <?php endif; ?>
            </p>
        </div>
        <?php if ($access_level == \pachno\core\framework\Settings::ACCESS_FULL): ?>
            <div class="lightyellowbox" style="padding: 5px; margin: 10px 0;<?php if (!\pachno\core\framework\Context::getScope()->hasProjectsAvailable()): ?> display: none;<?php endif; ?>" id="add_project_div">
                <form accept-charset="<?php echo \pachno\core\framework\Context::getI18n()->getCharset(); ?>" method="post" id="add_project_form" onsubmit="Pachno.Project.add('<?php echo make_url('configure_projects_add_project'); ?>');return false;">
                    <div style="height: 25px;">
                        ...
                        <div style="text-align: right; float: right;" class="button-group">
                            ...
                            <input type="submit" value="<?php echo __('Create project'); ?>" class="button first">
                            <input type="button" class="button last" onclick="$('add_project_additional').toggle();$(this).toggleClassName('button-pressed');" value="&#x25BC;">
                        </div>
                    </div>
                    <div id="add_project_additional" style="display: none; margin-top: 15px; font-size: 0.9em;">
                        ...
                        <select name="workflow_scheme_id" id="add_project_workflow_scheme">
                            <?php foreach (\pachno\core\entities\WorkflowScheme::getAll() as $workflowscheme): ?>
                        ...
                        <select name="issuetype_scheme_id" id="add_project_issuetype_scheme">
                            <?php foreach (\pachno\core\entities\IssuetypeScheme::getAll() as $issuetypescheme): ?>
