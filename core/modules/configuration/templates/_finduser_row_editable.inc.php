<?php if ($user->isScopeConfirmed()): ?>
    <form action="<?= make_url('configure_users_update_user', array('user_id' => $user->getID())); ?>" method="post" onsubmit="Pachno.Config.User.update('<?= make_url('configure_users_update_user', array('user_id' => $user->getID())); ?>', '<?= $user->getID(); ?>');return false;" id="edit_user_<?= $user->getID(); ?>_form" class="fullpage_backdrop">
        <div class="fullpage_backdrop_content backdrop_box large">
            ...
            <div class="backdrop_detail_content">
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td><?php if (\pachno\core\framework\Settings::isUsingExternalAuthenticationBackend()): echo $user->getUsername(); else: ?><input type="text" name="username" id="username_<?= $user->getID(); ?>" style="width: 120px;" value="<?= $user->getUsername(); ?>"><?php endif; ?></td>
                    ...
                    <tr>
                        ...
                        <td><?php if (\pachno\core\framework\Settings::isUsingExternalAuthenticationBackend()): if ($user->getRealname() == null): echo '-'; else: echo $user->getRealname(); endif; else: ?><input type="text" name="realname" id="realname_<?= $user->getID(); ?>" style="width: 220px;" value="<?= $user->getRealname(); ?>"><?php endif; ?></td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><?php if (\pachno\core\framework\Settings::isUsingExternalAuthenticationBackend()): if ($user->getNickname() == null): echo '-'; else: echo $user->getNickname(); endif; else: ?><input type="text" name="nickname" id="nickname_<?= $user->getID(); ?>" style="width: 220px;" value="<?= $user->getNickname(); ?>"><?php endif; ?></td>
                    </tr>
                    <tr>
                        <td><label for="email_<?= $user->getID(); ?>"><?= __('Email address'); ?></label></td>
                        <td><?php if (\pachno\core\framework\Settings::isUsingExternalAuthenticationBackend()): if ($user->getEmail() == null): echo '-'; else: echo $user->getEmail(); endif; else: ?><input type="text" name="email" id="email_<?= $user->getID(); ?>" style="width: 220px;" value="<?= $user->getEmail(); ?>"><?php endif; ?></td>
            ...
            <div class="backdrop_details_submit">
                ...
                <div class="submit_container">
                    <button type="submit" class="button"><?= image_tag('spinning_16.gif', ['id' => 'edit_user_' . $user->getID() . '_indicator', 'style' => 'display: none;']) . __('Update user'); ?></button>
                </div>
            </div>
        </div>
    </form>
