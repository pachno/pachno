    ...
    <div style="padding: 5px; position: relative;">
        ...
        <div style="position: absolute; right: 12px; top: 12px;">
            ...
            <ul id="team_<?= $team->getID(); ?>_more_actions_dropdown" style="font-size: 1.1em; width: 200px; top: 23px; margin-top: 0; text-align: right; z-index: 1000;" class="simple-list rounded_box white shadowed popup_box more_actions_dropdown" onclick="$(this).previous().toggleClassName('button-pressed');$(this).toggle();">
                ...
                <li>
                    <?= javascript_link_tag(__('List users in this team'), array('onclick' => 'Pachno.Config.Team.showMembers(\''.make_url('configure_users_get_team_members', array('team_id' => $team->getID())).'\', '.$team->getID().');')); ?>
                </li>
                <li>
                    <?= javascript_link_tag(__('Edit permissions for this team'), array('onclick' => "Pachno.Config.Team.getPermissionsBlock('".make_url('configure_permissions_get_configurator', array('team_id' => $team->getID(), 'base_id' => $team->getID())). "', ".$team->getID().");", 'id' => 'team_permissions_'.$team->getID().'_link')); ?>
                </li>
                <li>
        ...
        <div class="rounded_box white shadowed" style="margin: 5px; display: none;" id="clone_team_<?= $team->getID(); ?>">
            ...
            <div class="dropdown_content copy_team_link">
                <form id="clone_team_<?= $team->getID(); ?>_form" action="<?= make_url('configure_users_clone_team', array('team_id' => $team->getID())); ?>" method="post" accept-charset="<?= \pachno\core\framework\Settings::getCharset(); ?>" onsubmit="Pachno.Config.Team.clone('<?= make_url('configure_users_clone_team', array('team_id' => $team->getID())); ?>');return false;">
                    <div id="add_team">
                ...
                <div style="text-align: right;">
                    <?= javascript_link_tag(__('Clone this team'), array('onclick' => 'Pachno.Config.Team.clone(\''.make_url('configure_users_clone_team', array('team_id' => $team->getID())).'\', '.$team->getID().');')); ?> :: <b><?= javascript_link_tag(__('Cancel'), array('onclick' => '$(\'clone_team_'.$team->getID().'\').toggle();')); ?></b>
                </div>
