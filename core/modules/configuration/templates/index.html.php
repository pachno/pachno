...
<div class="configuration_container">
    <div class="configuration_update_check_container">
        ...
        <a class="button" id="update_button" href="javascript:void(0);" onclick="Pachno.Config.updateCheck('<?php echo make_url('configure_update_check'); ?>');"><?php echo __('Check for updates now'); ?></a>
    ...
    <?php endforeach; ?>
    <?php foreach ($config_sections[\pachno\core\framework\Settings::CONFIGURATION_SECTION_MODULES] as $section => $info): ?>
        <?php if ($info['module'] != 'core' && !\pachno\core\framework\Context::getModule($info['module'])->hasConfigSettings()) continue; ?>
        <li class="rounded_box">
        <?php if (is_array($info['route'])): ?>
            <?php $url = make_url($info['route'][0], $info['route'][1]); ?>
        <?php else: ?>
