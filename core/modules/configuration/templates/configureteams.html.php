    ...
    <div class="configuration-container">
        ...
        <div id="usersteamsgroups_menu_panes">
            <div id="tab_users_pane" style="padding-top: 0; width: 100%;">
                ...
                <div class="button-group">
                    <?php foreach (range('A', 'Z') as $letter): ?>
                        <?= javascript_link_tag($letter, array('class' => 'button', 'style' => 'width: 12px;', 'onclick' => "Pachno.Config.User.show('".make_url('configure_users_find_user')."', '{$letter}');")); ?>
                    <?php endforeach; ?>
                ...
                <div id="users_more_actions_container" style="position: relative;">
                    ...
                    <ul id="users_more_actions_dropdown" style="width: 400px; font-size: 1.1em; z-index: 1000; margin-top: 21px;" class="simple-list rounded_box white shadowed popup_box more_actions_dropdown">
                        ...
                        <li><?= javascript_link_tag(__('Show all users'), array('onclick' => "Pachno.Config.User.show('".make_url('configure_users_find_user')."', 'all');")); ?></li>
                        <li><?= javascript_link_tag(__('Show unactivated users'), array('onclick' => "Pachno.Config.User.show('".make_url('configure_users_find_user')."', 'unactivated');")); ?></li>
                        <li><?= javascript_link_tag(__('Show newly created users'), array('onclick' => "Pachno.Config.User.show('".make_url('configure_users_find_user')."', 'newusers');")); ?></li>
                    </ul>
                </div>
                        ...
                        <form action="<?= make_url('configure_users_add_user'); ?>" method="post" onsubmit="Pachno.Config.User.add('<?= make_url('configure_users_add_user'); ?>', import_cb);return false;" id="createuser_form">
                                    ...
                                    <dd>
                                        <select name="group_id">
                                            <?php foreach ($groups as $group): ?>
                                                <option value="<?= $group->getID(); ?>" <?php if ($group->getID() == \pachno\core\framework\Settings::getDefaultGroup()->getID()) echo ' selected'; ?>><?= $group->getName(); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </dd>
                                    ...
                                    <dd>
                                        <?php foreach ($teams as $team): ?>
                                            <div class="teamlist_container">
                                                <input type="checkbox" class="fancy-checkbox" id="adduser_teams_<?= $team->getID(); ?>" name="teams[<?= $team->getID(); ?>]" value="<?= $team->getID(); ?>">&nbsp;<label for="adduser_teams_<?= $team->getID(); ?>"><?= fa_image_tag('check-square', ['class' => 'checked'], 'far') . fa_image_tag('square', ['class' => 'unchecked'], 'far') . $team->getName(); ?></label>
                                            </div>
                                        <?php endforeach; ?>
                                    </dd>
                                </dl>
                            ...
                            <div class="backdrop_details_submit">
                                ...
                                <div class="submit_container">
                                    <button type="submit" class="button"><?= image_tag('spinning_16.gif', array('style' => 'display: none;', 'id' => 'createuser_form_indicator')) . ((\pachno\core\framework\Context::getScope()->isDefault()) ? __('Create user') : __('Create or add user')); ?></button>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <div id="tab_teams_pane" style="display: none;">
                <div class="lightyellowbox" style="margin-top: 5px; padding: 7px;<?php if (!\pachno\core\framework\Context::getScope()->hasTeamsAvailable()): ?> display: none;<?php endif; ?>" id="add_team_div">
                    <form id="create_team_form" action="<?= make_url('configure_users_add_team'); ?>" method="post" accept-charset="<?= \pachno\core\framework\Settings::getCharset(); ?>" onsubmit="Pachno.Config.Team.add('<?= make_url('configure_users_add_team'); ?>');return false;">
            ...
            <div id="tab_clients_pane" style="display: none;">
                <div class="lightyellowbox" style="margin-top: 5px; padding: 7px;">
                    <form id="create_client_form" action="<?= make_url('configure_users_add_client'); ?>" method="post" accept-charset="<?= \pachno\core\framework\Settings::getCharset(); ?>" onsubmit="Pachno.Config.Client.add('<?= make_url('configure_users_add_client'); ?>');return false;">
                        <div id="add_client">
