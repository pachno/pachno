...
<?php include_component('leftmenu', array('selected_section' => \pachno\core\framework\Settings::CONFIGURATION_SECTION_LICENSE)); ?>
<div valign="top" class="main_area main_configuration_content">
    <div style="width: 730px;" id="config_authentication">
        ...
        <div class="message-box type-info">
            ...
            <span class="actions"><a href="https://pachno.com/register/self-hosted" target="_blank" class="button button-purchase"><?= __('Purchase a subscription'); ?></a></span>
        </div>
        <?php if ($access_level == \pachno\core\framework\Settings::ACCESS_FULL): ?>
            <form accept-charset="<?= \pachno\core\framework\Context::getI18n()->getCharset(); ?>" action="<?= make_url('configure_license'); ?>" method="post" id="configure_license_form" onsubmit="Pachno.Main.Helpers.formSubmit('<?= make_url('configure_license'); ?>', 'configure_license_form'); return false;">
        <?php endif; ?>
            <table style="clear: both; width: 100%; margin-top: 15px;" class="padded_table" cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td>
                        <input type="text" name="license_key" id="license_key" value="<?= pachno\core\framework\Settings::getLicenseIdentifier(); ?>" class="padded">
                    </td>
                </tr>
                <tr>
