<?php if ($role instanceof \pachno\core\entities\Role): ?>
    <li class="greybox" style="padding: 3px 5px; margin-bottom: 5px;" id="role_<?php echo $role->getID(); ?>_container">
        <div class="button-group" style="float: right; margin-right: -3px; margin-top: -1px;">
            <?php echo javascript_link_tag(__('Details'), array('onclick' => "Pachno.Config.Roles.getPermissions('".make_url('configure_role', array('role_id' => $role->getID(), 'mode' => 'list_permissions'))."', 'role_{$role->getID()}_permissions_list');", 'class' => 'button')); ?>
            <?php if (!\pachno\core\framework\Context::isProjectContext() || !$role->isSystemRole()): ?>
                <?php echo javascript_link_tag(__('Edit'), array('onclick' => "Pachno.Config.Roles.getPermissionsEdit('".make_url('configure_role', array('role_id' => $role->getID(), 'mode' => 'edit'))."', 'role_{$role->getID()}_permissions_edit');", 'class' => 'button')); ?>
