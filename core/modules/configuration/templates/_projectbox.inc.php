...
<div id="project_box_<?= $project->getID();?>" class="greybox" style="margin: 10px 0px 10px 0px; position: relative;">
    ...
    <?php if ($project->hasDescription()): ?>
        <div colspan="2" style="padding: 3px;"><?= \pachno\core\helpers\TextParser::parseText($project->getDescription()); ?></div>
    <?php endif; ?>
    <?php if ($access_level == \pachno\core\framework\Settings::ACCESS_FULL): ?>
        <div style="position: absolute; right: 20px; top: 20px;">
            ...
            <button class="button dropper" id="project_<?= $project->getID(); ?>_more_actions"><?= __('Actions'); ?></button>
            <ul id="project_<?= $project->getID(); ?>_more_actions_dropdown" style="font-size: 1.1em; width: 200px; top: 23px; margin-top: 0; text-align: right; z-index: 1000;" class="simple-list rounded_box white shadowed popup_box more_actions_dropdown" onclick="$(this).previous().toggleClassName('button-pressed');$(this).toggle();">
                <?php if (!$project->isArchived()): ?>
                    <li><?= javascript_link_tag((($access_level == \pachno\core\framework\Settings::ACCESS_FULL) ? __('Edit project') : __('Show project details')), array('onclick' => "Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'project_config', 'project_id' => $project->getID()))."');")); ?></li>
                <?php endif; ?>
                <li><?= javascript_link_tag((($access_level == \pachno\core\framework\Settings::ACCESS_FULL) ? __('Edit project permissions') : __('Show project permissions')), array('onclick' => "$('project_{$project->getID()}_permissions').toggle();")); ?></li>
                <li id="project_<?= $project->getID(); ?>_unarchive" style="<?php if (!$project->isArchived()) echo 'display: none;'; ?>"><a href="javascript:void(0)" onclick="Pachno.Project.unarchive('<?= make_url('configure_project_unarchive', array('project_id' => $project->getID())); ?>', <?php print $project->getID(); ?>)"><?= __('Unarchive project');?></a></li>
