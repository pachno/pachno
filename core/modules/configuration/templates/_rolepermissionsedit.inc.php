<form action="<?php echo make_url('configure_role', array('role_id' => $role->getID(), 'mode' => 'edit')); ?>" id="role_<?php echo $role->getID(); ?>_form" method="post" onsubmit="Pachno.Config.Roles.update('<?php echo make_url('configure_role', array('role_id' => $role->getID(), 'mode' => 'edit')); ?>', <?php echo $role->getID(); ?>);return false;">
    ...
    <input type="text" id="role_<?php echo $role->getID(); ?>_name" name="name" value="<?php echo htmlentities($role->getName(), ENT_COMPAT, \pachno\core\framework\Context::getI18n()->getCharset()); ?>" style="width: 300px;"><br>
    <div style="margin-top: 10px; font-weight: bold;"><?php echo __('Role permissions'); ?></div>
    <ul class="simple-list" style="display: block; width: auto;">
