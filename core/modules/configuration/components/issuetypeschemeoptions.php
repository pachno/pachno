            ...
            <span class="column">
                ...
                <div class="filter-values-container">
                    <?php foreach ($builtin_fields as $item): ?>
                        ...
                        <a href="javascript:void(0);" class="list-item filtervalue <?php if (array_key_exists($item, $visible_fields)) echo 'disabled'; ?>" data-issue-field data-id="<?= $item; ?>" data-url="<?= make_url('configure_issuetypes_scheme_field', ['key' => $item, 'scheme_id' => $scheme->getId(), 'issue_type_id' => $issue_type->getId()]); ?>">
            ...
            <span class="column">
                ...
                <div class="filter-values-container">
                    <?php foreach ($custom_fields as $item): ?>
                        <a href="javascript:void(0);" class="list-item filtervalue <?php if (array_key_exists($item->getKey(), $visible_fields)) echo 'disabled'; ?>" data-issue-field data-id="<?= $item->getKey(); ?>" data-url="<?= make_url('configure_issuetypes_scheme_field', ['key' => $item->getKey(), 'scheme_id' => $scheme->getId(), 'issue_type_id' => $issue_type->getId()]); ?>">
