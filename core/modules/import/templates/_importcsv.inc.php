    ...
    <form accept-charset="<?= \pachno\core\framework\Context::getI18n()->getCharset(); ?>" method="post" id="import_csv_form" onsubmit="Pachno.Config.Import.importCSV('<?= make_url('import_do_import_csv', array('type' => $type)); ?>');return false;">
        ...
        <div class="backdrop_details_submit" id="csv_import_control">
            <span class="explanation">
                <input type="checkbox" class="fancy-checkbox" name="csv_dry_run" id="csv_dry_run" checked="checked"><label for="csv_dry_run"><?= fa_image_tag('check-square', ['class' => 'checked'], 'far') . fa_image_tag('square', ['class' => 'unchecked'], 'far') . __('Test the import (dry-run)'); ?></label>
            </span>
