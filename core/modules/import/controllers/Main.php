<?php
    ...
    namespace pachno\core\modules\import\controllers;
    ...
    class Main extends framework\Action
    {
        ...
        public function runIndex(framework\Request $request)
        {
            if ($request->isPost()) {
                if ($request['import_sample_data']) {
                    ...
                    $lorem_ipsum = tables\Articles::getTable()->getArticleByName('LoremIpsum');
                    $lorem_words = explode(' ', $lorem_ipsum->getContent());

                    foreach (['bugreport', 'featurerequest', 'enhancement', 'idea'] as $issuetype) {
                        ...
                        for ($cc = 1; $cc <= 10; $cc++) {
                            ...
                            for ($ll = 1; $ll <= $rand_length; $ll++) {
                                $word = str_replace([',', '.', "\r", "\n"], ['', '', '', ''], $lorem_words[array_rand($lorem_words)]);
        ...
        {
            $content = $this->getComponentHTML('import/importcsv', ['type' => $request['type']]);
        ...
        public function runDoImportCSV(framework\Request $request)
        {
            try {
                ...
                while (($row = fgetcsv($handle, 1000)) !== false) {
                    if (!$headerrow) {
                        ...
                        if (count($headerrow) == count($row)) {
                            ...
                            $errors[] = $this->getI18n()->__('Row %row does not have the same number of elements as the header row', ['%row' => $i]);
                        }
                    }
                    $i++;
                }
                ...
                $requiredcols = [
                    self::CSV_TYPE_CLIENTS => [self::CSV_CLIENT_NAME],
                    self::CSV_TYPE_PROJECTS => [self::CSV_PROJECT_NAME],
                    self::CSV_TYPE_ISSUES => [self::CSV_ISSUE_TITLE, self::CSV_ISSUE_PROJECT, self::CSV_ISSUE_ISSUE_TYPE],
                ];
                ...
                foreach ($requiredcols[$request['type']] as $col) {
                    if (!in_array($col, $headerrow)) {
                        $errors[] = $this->getI18n()->__('Required column \'%col\' not found in header row', ['%col' => $col]);
                    }
                }
                ...
                for ($i = 0; $i != count($data); $i++) {
                    ...
                    foreach ($activerow as $col => $val) {
                        if (strlen($val) == 0) {
                            $errors[] = $this->getI18n()->__('Row %row column %col has no value', ['%col' => $col, '%row' => $i + 1]);
                        }
                    }
                }
                    ...
                    switch ($request['type']) {
                        case self::CSV_TYPE_PROJECTS:
                            for ($i = 0; $i != count($data); $i++) {
                                ...
                                if ($tmp !== null) {
                                    $errors[] = $this->getI18n()->__('Row %row: A project with this name already exists', ['%row' => $i + 1]);
                                }

                                // First off are booleans
                                $boolitems = [self::CSV_PROJECT_SCRUM, self::CSV_PROJECT_ALLOW_REPORTING, self::CSV_PROJECT_AUTOASSIGN, self::CSV_PROJECT_FREELANCE,
                                ...
                                foreach ($boolitems as $boolitem) {
                                    if (array_key_exists($boolitem, $activerow) && isset($activerow[$boolitem]) && $activerow[$boolitem] != 1 && $activerow[$boolitem] != 0) {
                                        $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be 1/0)', ['%col' => $boolitem, '%row' => $i + 1]);
                                    }
                                }

                                // Now identifiables
                                $identifiableitems = [
                                    [self::CSV_PROJECT_QA, self::CSV_PROJECT_QA_TYPE],
                                    [self::CSV_PROJECT_LEAD, self::CSV_PROJECT_LEAD_TYPE],
                                    [self::CSV_PROJECT_OWNER, self::CSV_PROJECT_OWNER_TYPE]
                                ];
                                ...
                                foreach ($identifiableitems as $identifiableitem) {
                                    ...
                                    if ((!array_key_exists($identifiableitem[1], $activerow) && array_key_exists($identifiableitem[0], $activerow)) || (array_key_exists($identifiableitem[1], $activerow) && !array_key_exists($identifiableitem[0], $activerow))) {
                                        $errors[] = $this->getI18n()->__('Row %row: Both the type and item ID must be supplied for owner/lead/qa fields', ['%row' => $i + 1]);
                                        continue;
                                    }
                                    ...
                                    if (array_key_exists($identifiableitem[1], $activerow) && isset($activerow[$identifiableitem[1]]) !== null && $activerow[$identifiableitem[1]] != self::CSV_IDENTIFIER_TYPE_USER && $activerow[$identifiableitem[1]] != self::CSV_IDENTIFIER_TYPE_TEAM) {
                                        $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be 1 for a user or 2 for a team)', ['%col' => $identifiableitem[1], '%row' => $i + 1]);
                                    }
                                    ...
                                    if (array_key_exists($identifiableitem[0], $activerow) && isset($activerow[$identifiableitem[0]]) && !is_numeric($activerow[$identifiableitem[0]])) {
                                        $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be a number)', ['%col' => $identifiableitem[0], '%row' => $i + 1]);
                                ...
                                if (array_key_exists(self::CSV_PROJECT_SUMMARY_TYPE, $activerow) && isset($activerow[self::CSV_PROJECT_SUMMARY_TYPE])) {
                                    if ($activerow[self::CSV_PROJECT_SUMMARY_TYPE] != 'issuetypes' && $activerow[self::CSV_PROJECT_SHOW_SUMMARY] != 'milestones') {
                                        $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be \'issuetypes\' or \'milestones\')', ['%col' => self::CSV_PROJECT_SUMMARY_TYPE, '%row' => $i + 1]);
                                    }
                                }
                            }
                            break;
                        case self::CSV_TYPE_ISSUES:
                            for ($i = 0; $i != count($data); $i++) {
                                ...
                                // First off are booleans
                                $boolitems = [self::CSV_ISSUE_STATE, self::CSV_ISSUE_BLOCKING];
                                ...
                                // Now numerics
                                $numericitems = [self::CSV_ISSUE_VOTES, self::CSV_ISSUE_PERCENTAGE, self::CSV_ISSUE_ISSUENO];
                                ...
                                foreach ($numericitems as $numericitem) {
                                    if (array_key_exists($numericitem, $activerow) && isset($activerow[$numericitem]) && !(is_numeric($activerow[$numericitem]))) {
                                        $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be a number)', ['%col' => $numericitem, '%row' => $i + 1]);
                                    }
                                }
                                ...
                                if (array_key_exists(self::CSV_ISSUE_PERCENTAGE, $activerow) && isset($activerow[self::CSV_ISSUE_PERCENTAGE]) && (($activerow[self::CSV_ISSUE_PERCENTAGE] < 0) || ($activerow[self::CSV_ISSUE_PERCENTAGE] > 100))) {
                                    $errors[] = $this->getI18n()->__('Row %row column %col: Percentage must be from 0 to 100 inclusive', ['%col' => self::CSV_ISSUE_PERCENTAGE, '%row' => $i + 1]);
                                }

                                // Now identifiables
                                $identifiableitems = [
                                    [self::CSV_ISSUE_OWNER, self::CSV_ISSUE_OWNER_TYPE],
                                    [self::CSV_ISSUE_ASSIGNED, self::CSV_ISSUE_ASSIGNED_TYPE]
                                ];
                                ...
                                if (array_key_exists(self::CSV_ISSUE_POSTED, $activerow) && isset($activerow[self::CSV_ISSUE_POSTED]) && ((string)(int)$activerow[self::CSV_ISSUE_POSTED] !== $activerow[self::CSV_ISSUE_POSTED]) && $activerow[self::CSV_ISSUE_POSTED] >= PHP_INT_MAX && $activerow[self::CSV_ISSUE_POSTED] <= ~PHP_INT_MAX) {
                                    $errors[] = $this->getI18n()->__('Row %row column %col: invalid value (must be a Unix timestamp)', ['%col' => self::CSV_ISSUE_POSTED, '%row' => $i + 1]);
                                }
                                ...
                                if (array_key_exists(self::CSV_ISSUE_MILESTONE, $activerow) && isset($activerow[self::CSV_ISSUE_MILESTONE])) {
                                    if (!is_numeric($activerow[self::CSV_ISSUE_MILESTONE])) {
                                        ...
                                        try {
                                            ...
                                            $errors[] = $this->getI18n()->__('Row %row column %col: milestone does not exist', ['%col' => self::CSV_ISSUE_MILESTONE, '%row' => $i + 1]);
                                        }
                                    }
                                }
                                ...
                                if (array_key_exists(self::CSV_ISSUE_ISSUE_TYPE, $activerow) && isset($activerow[self::CSV_ISSUE_ISSUE_TYPE])) {
                                    if (!is_numeric($activerow[self::CSV_ISSUE_ISSUE_TYPE])) {
                                        ...
                                        try {
                                            ...
                                            $errors[] = $this->getI18n()->__('Row %row column %col: issue type does not exist', ['%col' => self::CSV_ISSUE_ISSUE_TYPE, '%row' => $i + 1]);
                                        }
                                    }
                                }
                            }
                ...
                if (count($errors) != 0) {
                    ...
                    return $this->renderJSON(['errordetail' => $errordiv, 'error' => $this->getI18n()->__('Errors occured while importing, see the error list in the import screen for further details')]);
                }
            } catch (Exception $e) {
                $this->getResponse()->setHttpStatus(400);

                return $this->renderJSON(['errordetail' => $e->getMessage(), 'error' => $e->getMessage()]);
            }

            if ($request['csv_dry_run']) {
                return $this->renderJSON(['message' => $this->getI18n()->__('Dry-run successful, you can now uncheck the dry-run box and import your data.')]);
            } else {
                switch ($request['type']) {
                    case self::CSV_TYPE_CLIENTS:
                        for ($i = 0; $i != count($data); $i++) {
                            try {
                                ...
                                $errors[] = $this->getI18n()->__('Row %row failed: %err', ['%row' => $i + 1, '%err' => $e->getMessage()]);
                            }
                        }
                        break;
                    case self::CSV_TYPE_PROJECTS:
                ...
                if (count($errors) != 0) {
                    ...
                    return $this->renderJSON(['message' => $this->getI18n()->__('Successfully imported %num rows!', ['%num' => count($data)])]);
                }
            }
        }
