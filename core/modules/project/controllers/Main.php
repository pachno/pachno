...
class Main extends helpers\ProjectActions
{
    ...
    public function runSortMilestones(framework\Request $request)
    {
        ...
        try
        {
            if (is_array($milestones))
            {
                ...
                {
                    $milestone = \pachno\core\entities\tables\Milestones::getTable()->selectByID($milestone_id);
    ...
    public function runStatisticsLast15(framework\Request $request)
    {
        ...
        {
            return $this->return404(framework\Context::getI18n()->__('The libraries to generate images are not installed. Please see https://pachno.com for more information'));
        }
    ...
    public function runTransitionIssues(framework\Request $request)
    {
        try
        {
            ...
            framework\Context::loadLibrary('common');
            $options = array('issue_ids' => array_keys($issue_ids), 'last_updated' => \pachno\core\framework\Context::getI18n()->formatTime(time(), 20), 'closed' => $closed);
