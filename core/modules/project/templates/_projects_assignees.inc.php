    ...
    <tbody id="project_team_users">
        <?php foreach ($assigned_users as $user): ?>
            <tr id="assignee_user_<?php echo $user->getID(); ?>_row" class="hoverable">
                <td style="width: 20px;">
                    <a href="javascript:void(0);" id="assignee_user_<?php echo $user->getID(); ?>_link" class="delete" onclick="Pachno.Main.Helpers.Dialog.show('<?php echo __('Remove %username from this project?', array('%username' => $user->getNameWithUsername())); ?>', '<?php echo __('Please confirm that you want to remove this user from the project team'); ?>', {yes: {click: function() {Pachno.Project.removeAssignee('<?php echo make_url('configure_project_remove_assignee', array('project_id' => $project->getID(), 'assignee_type' => 'user', 'assignee_id' => $user->getID())); ?>', 'user', <?php echo $user->getID(); ?>);Pachno.Main.Helpers.Dialog.dismiss();}}, no: {click: Pachno.Main.Helpers.Dialog.dismiss}});"><?php echo fa_image_tag('times'); ?></a>
    ...
    <tbody id="project_team_teams">
        <?php foreach ($assigned_teams as $team): ?>
            <tr id="assignee_team_<?php echo $team->getID(); ?>_row" class="hoverable">
                <td style="width: 20px;">
                    <a href="javascript:void(0);" id="assignee_team_<?php echo $team->getID(); ?>_link" onclick="Pachno.Main.Helpers.Dialog.show('<?php echo __('Remove %teamname from this project?', array('%teamname' => $team->getName())); ?>', '<?php echo __('Please confirm that you want to remove this team from the project team'); ?>', {yes: {click: function() {Pachno.Project.removeAssignee('<?php echo make_url('configure_project_remove_assignee', array('project_id' => $project->getID(), 'assignee_type' => 'team', 'assignee_id' => $team->getID())); ?>', 'team', <?php echo $team->getID(); ?>);Pachno.Main.Helpers.Dialog.dismiss();}}, no: {click: Pachno.Main.Helpers.Dialog.dismiss}});"><?php echo image_tag('action_delete.png'); ?></a>
