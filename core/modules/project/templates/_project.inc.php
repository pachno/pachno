<?php
    ...
    if ($pachno_user->hasPageAccess('project_timeline', $project->getID()) || $pachno_user->hasPageAccess('project_allpages', $project->getID()))
    {
        $pachno_response->addFeed(make_url('project_timeline', array('project_key' => $project->getKey(), 'format' => 'rss')), __('"%project_name" project timeline', array('%project_name' => $project->getName())));
    }
...
<div class="project_strip">
    ...
    <nav class="button-group">
        ...
        <?php endif; ?>
        <?php \pachno\core\framework\Event::createNew('core', 'project_overview_item_links', $project)->trigger(); ?>
        ...
        <?php endif; ?><?php if (!$project->isLocked() && $pachno_user->canReportIssues($project)): ?>
            <?= javascript_link_tag(fa_image_tag('plus-square') . '<span>'.__('New issue').'</span>', ['onclick' => "Pachno.Issues.Add('" . make_url('get_partial_for_backdrop', ['key' => 'reportissue', 'project_id' => $project->getId()]) . "', this);", 'class' => 'nav-button button-report-issue']); ?>
        <?php endif; ?>
    </nav>
</div>
