<?php if (isset($roles)): ?>
    <div id="project<?php echo $project->getID(); ?>_settings_roles" style="position: relative;">
        ...
        <div class="content faded_out">
            <p><?php echo __("Roles are applied when assigning users or teams to a project, granting them access to specific parts of the project or giving users access to update and edit information. Updating permissions in this list will add or remove permissions for all users and / or team members with that role, on all assigned projects. Removing a role removes all permissions granted by that role for all users and teams. Read more about roles and permissions in the %online_documentation", array('%online_documentation' => link_tag('https://projects.pachno.com/pachno/docs/RolesAndPermissions', '<b>'.__('online documentation').'</b>'))); ?></p>
        </div>
        ...
        <div class="rounded_box white shadowed" id="new_project<?php echo $project->getID(); ?>_role" style="display: none; position: absolute; right: 0; z-index: 10">
            <form id="new_project<?php echo $project->getID(); ?>_role_form" method="post" action="<?php echo make_url('project_create_role', array('project_key' => $project->getKey())); ?>" onsubmit="Pachno.Project.Roles.add('<?php echo make_url('project_create_role', array('project_key' => $project->getKey())); ?>', <?php echo $project->getID(); ?>); return false;" accept-charset="<?php echo \pachno\core\framework\Context::getI18n()->getCharset(); ?>">
...
<div class="permission_list" id="project<?php echo $project->getID(); ?>_settings_advanced_permissions"<?php if (isset($roles)): ?> style="display: none;"<?php endif; ?>>
    <?php if (isset($roles)): ?>
        <div class="project_save_container">
            <div class="button" onclick="$('project<?php echo $project->getID(); ?>_settings_roles').toggle();$('project<?php echo $project->getID(); ?>_settings_advanced_permissions').toggle();"><?php echo __('Show roles'); ?></div>
        </div>
    <?php endif; ?>
    <h3>
    ...
    <div class="content faded_out">
        <p><?php echo __('These permissions directly control what you can do, and which pages you can access in Pachno - on a project-specific basis. Some of these permissions are also available as site-wide permissions in the %permissions_configuration page. You may want to use roles and assignments instead of applying these permissions directly.', array('%permissions_configuration' => '<b>'.link_tag(make_url('configure_permissions'), __('permissions configuration')).'</b>')); ?></p>
    </div>
    <ul id="project_permission_details_<?php echo $project->getID(); ?>" style="margin-top: 10px;">
        <?php include_component('configuration/permissionsblock', array('base_id' => 0 . 'project_' . $project->getID() . '_project_permissions', 'permissions_list' => \pachno\core\framework\Context::getAvailablePermissions('project'), 'mode' => 'general', 'target_id' => $project->getID(), 'module' => 'core', 'access_level' => $access_level)); ?>
        <?php //include_component('configuration/permissionsblock', array('base_id' => 0 . 'project_' . $project->getID() . '_page_permissions', 'permissions_list' => \pachno\core\framework\Context::getAvailablePermissions('project_pages'), 'mode' => 'project_pages', 'target_id' => $project->getID(), 'module' => 'core', 'access_level' => $access_level)); ?>
        <?php include_component('configuration/permissionsblock', array('base_id' => 0 . 'project_' . $project->getID() . '_issue_permissions', 'permissions_list' => \pachno\core\framework\Context::getAvailablePermissions('issues'), 'mode' => 'general', 'target_id' => $project->getID(), 'module' => 'core', 'access_level' => $access_level)); ?>
    </ul>
</div>
