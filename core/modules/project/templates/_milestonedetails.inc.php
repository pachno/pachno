<?php if ($milestone instanceof \pachno\core\entities\Milestone): ?>
    <div class="milestone_details" id="milestone_details_<?php echo $milestone->getID(); ?>">
        <h3>
            ...
            <?php echo javascript_link_tag(fa_image_tag('edit'), array('onclick' => "Pachno.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'milestone', 'project_id' => $milestone->getProject()->getId(), 'milestone_id' => $milestone->getID()))."');", 'class' => 'button button-icon button-silver')); ?>
            <br>
        </h3>
        <span class="milestone_date">
            <?php if ($milestone->getStartingDate() && $milestone->isScheduled()): ?>
                (<?php echo \pachno\core\framework\Context::getI18n()->formatTime($milestone->getStartingDate(), 22, true, true); ?> - <?php echo \pachno\core\framework\Context::getI18n()->formatTime($milestone->getScheduledDate(), 22, true, true); ?>)
            <?php elseif ($milestone->getStartingDate() && !$milestone->isScheduled()): ?>
                (<?php echo __('Starting %start_date', array('%start_date' => \pachno\core\framework\Context::getI18n()->formatTime($milestone->getStartingDate(), 22, true, true))); ?>)
            <?php elseif (!$milestone->getStartingDate() && $milestone->isScheduled()): ?>
                (<?php echo __('Ends %end_date', array('%end_date' => \pachno\core\framework\Context::getI18n()->formatTime($milestone->getScheduledDate(), 22, true, true))); ?>)
            <?php endif; ?>
        </span>
            ...
            <tbody>
                ...
                <?php if ($milestone->hasStartingDate() && $milestone->hasScheduledDate()): ?>
                    <tr>
                        ...
                        <td style="width: 50px; border-top: 1px dotted #AAA; border-bottom: 0; font-size: 13px; font-weight: bold; text-align: center; padding: 5px;" id="scrum_sprint_<?php echo $milestone->getID(); ?>_previous_spent_hours"><?php echo isset($burndown_data['spent_times']['hours_spent_before']) ? \pachno\core\entities\common\Timeable::formatHoursAndMinutes($burndown_data['spent_times']['hours_spent_before'], $burndown_data['spent_times']['minutes_spent_before']) : 0; ?></td>
                        <td style="padding: 5px; border-top: 1px dotted #AAA; border-bottom: 0; font-weight: bold; font-size: 12px;">&nbsp;</td>
                    </tr>
                <?php endif; ?>
                <tr>
                    ...
                    jQuery(function () {
                        ...
                        <?php if ($milestone->hasStartingDate() && $milestone->hasScheduledDate()): ?>
                            ...
                            var d_e_velocity_points = [[ep_keys.min() * 1000, ep_values.reduce(add, 0)], [ep_keys.max() * 1000, 0]];
                                ...
                                yaxis: Pachno.Chart.config.y_config,
                                xaxis: x_config,
                                grid: grid_config
                                });
                        <?php else: ?>
                            ...
                            jQuery.plot(jQuery("#selected_burndown_image"), [
                                ...
                                yaxis: Pachno.Chart.config.y_config,
                                xaxis: x_config,
                                grid: Pachno.Chart.config.grid_config
                            });
                        <?php endif; ?>
                    });
                });
