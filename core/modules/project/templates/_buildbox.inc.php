...
<li id="show_build_<?php print $b_id; ?>" class="release_item <?php if ($build->isActive()) echo 'active'; ?> <?php if ($build->hasDownload()) echo 'download'; ?>">
    <?php if ($pachno_user->canManageProjectReleases($build->getProject())): ?>
        <div class="button-group">
            <button class="button" onclick="Pachno.UI.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'project_build', 'project_id' => $build->getProject()->getId(), 'build_id' => $build->getId())); ?>');"><?php echo __('Edit'); ?></button>
            <button class="button" onclick="Pachno.UI.Dialog.show('<?php echo __('Delete this release?'); ?>', '<?php echo __('Do you really want to delete this release?').'<br>'.__('Deleting this release will make it unavailable for download, and remove it from any associated issue reports or feature requests.').'<br><b>'.__('This action cannot be reverted').'</b>'; ?>', {yes: {click: function() {Pachno.Project.Build.remove('<?php echo make_url('configure_build_delete', array('build_id' => $b_id, 'project_id' => $build->getProject()->getId())); ?>', <?php print $b_id; ?>, '<?php echo ($build->isLocked()) ? "archived" : "active"; ?>', <?php echo $build->getEditionID(); ?>);}}, no: {click: Pachno.UI.Dialog.dismiss}});"><?php echo __('Delete'); ?></button>
        </div>
    <?php endif; ?>
