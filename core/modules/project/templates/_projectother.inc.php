                    ...
                    <?php else: ?>
                        <?php foreach ($project->getMilestonesForRoadmap() as $milestone): ?>
                            <?php if ($milestone->isVisibleIssues() || $milestone->isVisibleRoadmap() || $project->isMilestoneVisible($milestone->getID())): ?>
                                <div style="clear: both; font-size: 12px;">
                                    <input type="checkbox" class="fancy-checkbox" name="showmilestone[<?php echo $milestone->getID(); ?>]" value="<?php echo $milestone->getID(); ?>"<?php if ($project->isMilestoneVisible($milestone->getID())): ?> checked<?php endif; ?> id="showmilestone_<?php echo $milestone->getID(); ?>" style="float: left;"<?php if ($access_level != \pachno\core\framework\Settings::ACCESS_FULL): ?> disabled<?php endif; ?>>
