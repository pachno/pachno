        ...
        <table style="clear: both; width: 780px;" class="padded_table" cellpadding=0 cellspacing=0>
            <tr>
                ...
                <td style="width: 580px;">
                    <?php if ($access_level == \pachno\core\framework\Settings::ACCESS_FULL): ?>
                        <input type="text" name="project_name" id="project_name_input" onblur="Pachno.Project.updatePrefix('<?= make_url('configure_project_get_updated_key', ['project_id' => $project->getID()]); ?>', <?= $project->getID(); ?>);" value="<?php print $project->getName(); ?>" style="width: 576px; padding: 4px; font-size: 1.2em;">
                    <?php else: ?>
                        <?= $project->getName(); ?>
                    <?php endif; ?>
                </td>
        ...
        <?php if ($access_level == \pachno\core\framework\Settings::ACCESS_FULL): ?>
            <div class="backdrop_details_submit">
                ...
                <div class="submit_container">
                    <?= image_tag('spinning_16.gif', ['id' => 'project_info_indicator', 'style' => 'display: none;']); ?>
                    <?php if ($project->getId()): ?>
                        <button class="button primary" id="project_submit_settings_button" onclick="Pachno.Project.submitInfo('<?= make_url('configure_project_settings', ['project_id' => $project->getID()]); ?>', <?= $project->getID(); ?>);"><?= fa_image_tag('save'); ?><span><?= __('Save project'); ?></span></button>
                    <?php else: ?>
                        <button class="button primary" id="project_submit_settings_button" onclick="Pachno.Project.submitInfo('<?= make_url('configure_project_settings', ['project_id' => $project->getID()]); ?>', <?= $project->getID(); ?>);"><?= fa_image_tag('plus-square'); ?><span><?= __('Create project'); ?></span></button>
                    <?php endif; ?>
                </div>
            </div>
        </form>
