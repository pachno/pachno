    ...
    <form accept-charset="<?php echo \pachno\core\framework\Context::getI18n()->getCharset(); ?>" action="<?php echo make_url('configure_projects_build', array('project_id' => $project->getID())); ?>" method="post" id="build_form" onsubmit="$('add_release_indicator').show();return true;" enctype="multipart/form-data">
                ...
                <tr>
                    ...
                    <td>
                        <select name="milestone" id="build_milestone_dropdown">
                            <option value="0"<?php if (!$build->getMilestone() instanceof \pachno\core\entities\Milestone) echo ' selected'; ?>><?php echo __('This release is not related to a milestone'); ?></option>
                            <?php foreach ($project->getAvailableMilestones() as $milestone): ?>
                                <option value="<?php echo $milestone->getID(); ?>"<?php if ($build->getMilestone() instanceof \pachno\core\entities\Milestone && $build->getMilestone()->getID() == $milestone->getID()) echo ' selected'; ?>><?php echo __('This is a release of milestone %milestone_name', array('%milestone_name' => $milestone->getName())); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="edition" id="build_edition_dropdown">
                                <option value="0"<?php if (!$build->getEdition() instanceof \pachno\core\entities\Edition) echo ' selected'; ?>><?php echo __('This release is not related to a edition'); ?></option>
                                <?php foreach ($project->getEditions() as $edition): ?>
                                    <option value="<?php echo $edition->getID(); ?>"<?php if ($build->getEdition() instanceof \pachno\core\entities\Edition && $build->getEdition()->getID() == $edition->getID()) echo ' selected'; ?>><?php echo __('This is a release of edition %edition_name', array('%edition_name' => $edition->getName())); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                    </tr>
        ...
        <div class="backdrop_details_submit">
            ...
            <div class="submit_container">
                ...
                <?php endif; ?>
                <button class="button" type="submit"><?php echo image_tag('spinning_20.gif', ['id' => 'add_release_indicator', 'style' => 'display: none;']) . (($build->getId()) ? __('Update release') : __('Add release')); ?></button>
            </div>
        </div>
    </form>
</div>
