...
<tr class="hover_highlight">
    <td style="font-weight: normal;" class="issue_title_container<?php if ($issue->isClosed()) echo ' faded_out'; ?>">
        ...
        <?php echo link_tag($issue->getUrl(), fa_image_tag($issue->getIssueType()->getFontAwesomeIcon(), ['title' => $issue->getIssueType()->getName()]) . $issue->getFormattedTitle(), array('title' => $issue->getFormattedTitle(), 'style' => 'width: ' . (550 - 30*($child)).'px')); ?>
    </td>
    <td class="estimates">
    ...
    <td class="milestone_issue_actions">
        <div>
            <a class="dropper button button-icon" id="more_actions_<?php echo $issue->getID(); ?>_button"><?php echo fa_image_tag('ellipsis-v', array('title' => __('Show more actions'))); ?></a>
