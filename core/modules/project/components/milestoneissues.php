...
<?php foreach ($milestone->getIssues() as $issue): ?>
    <?php if ($issue->hasAccess()): ?>
        <div class="roadmap_issue<?php if ($issue->isClosed()): ?> faded_out issue_closed<?php elseif ($issue->isBlocking()): ?> blocking<?php endif; ?>">
            <div class="issue_status"><div style="border: 1px solid #AAA; background-color: <?php echo ($issue->getStatus() instanceof \pachno\core\entities\Status) ? $issue->getStatus()->getColor() : '#FFF'; ?>; font-size: 1px; width: 13px; height: 13px;" title="<?php echo ($issue->getStatus() instanceof \pachno\core\entities\Status) ? $issue->getStatus()->getName() : ''; ?>">&nbsp;</div></div>
            ...
            <?php endif; ?>
            <?php echo link_tag($issue->getUrl(), $issue->getFormattedIssueNo(true) . ' - <span class="issue_title">' . $issue->getTitle() . '</span>'); ?>
