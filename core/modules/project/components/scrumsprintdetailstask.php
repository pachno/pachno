<tr class="hover_highlight" id="scrum_story_task_<?php echo $task->getID(); ?>">
    <td>&nbsp;</td>
    <td style="padding: 3px 0 3px 10px; font-size: 12px; <?php if ($task->isClosed()): ?> text-decoration: line-through; <?php endif; ?>"<?php if ($task->isClosed()): ?> class="faded_out"<?php endif; ?>><?php echo link_tag($issue->getUrl(), array('issue_no' => $task->getFormattedIssueNo(false), 'project_key' => $task->getProject()->getKey())), $task->getTitle()); ?></td>
