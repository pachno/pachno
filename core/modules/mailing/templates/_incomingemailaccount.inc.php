<div id="incoming_email_account_<?php echo $account->getID(); ?>">
    <h5>
        <div class="button-group" style="float: right; margin: 10px 0 0 5px;">
            ...
            <div class="button"><span onclick="Pachno.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'mailing_editincomingemailaccount', 'project_id' => $account->getProject()->getId(), 'account_id' => $account->getID())); ?>');"><?php echo __('Edit'); ?></span></div>
            <div class="button"><span onclick="Pachno.Main.Helpers.Dialog.show('<?php echo __('Really delete this incoming email account?'); ?>', '<?php echo __('Deleting this incoming email account will stop emails being retrieved from this account. All existing issues are still kept.'); ?>', { yes: { click: function() { Pachno.Modules.mailing.deleteIncomingAccount('<?php echo make_url('mailing_delete_account', array('account_id' => $account->getID())); ?>', <?php echo $account->getID(); ?>);}}, no: { click: Pachno.Main.Helpers.Dialog.dismiss }});"><?php echo __('Delete'); ?></span></div>
        </div>
