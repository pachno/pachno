<?php
    ...
    use pachno\core\entities\Article,
        pachno\core\entities\IncomingEmailAccount,
        pachno\core\entities\tables\MailQueueTable,
        Swift_Message,
        Swift_Mime_Message,
        Swift_Mailer,
        Swift_MailTransport,
    ...
    class Mailing extends CoreModule
    {
         ...
         *
         * @return \pachno\core\modules\mailing\Mailing
         */
        ...
        {
            framework\Event::listen('core', 'pachno\core\entities\User::_postSave', array($this, 'listen_registerUser'));
            ...
            framework\Event::listen('core', 'pachno\core\entities\User::addScope', array($this, 'listen_addScope'));
            framework\Event::listen('core', 'pachno\core\entities\Issue::createNew', array($this, 'listen_issueCreate'));
            framework\Event::listen('core', 'pachno\core\entities\User::_postSave', array($this, 'listen_createUser'));
            framework\Event::listen('core', 'pachno\core\entities\Issue::save', array($this, 'listen_issueSave'));
            framework\Event::listen('core', 'pachno\core\entities\Comment::createNew', array($this, 'listen_pachno_core_entities_Comment_createNew'));
            framework\Event::listen('core', 'pachno\core\entities\Comment::_postSave', array($this, 'listen_pachno_core_entities_Comment_createNew'));
            framework\Event::listen('core', 'pachno\core\entities\Article::doSave', array($this, 'listen_Article_doSave'));
            ...
            framework\Event::listen('core', 'pachno\core\framework\helpers\TextParser::_parseIssuelink::urlPrefix', array($this, 'listen_getMailingUrl'));
        }
...
EOT;
        ...
        public function getUsersAndLanguages($users)
        {
            ...
            {
                if (is_numeric($user))
                    $user = \pachno\core\entities\User::getB2DBTable()->selectById($user);
...
//                __('Forgot your password?');
        ...
        public function listen_Article_doSave(framework\Event $event)
        {
            ...
            $user = \pachno\core\entities\User::getB2DBTable()->selectById((int) $event->getParameter('user_id'));
        ...
        public function listen_pachno_core_entities_Comment_createNew(framework\Event $event)
        {
            if (!$this->isOutgoingNotificationsEnabled()) {
                return;
            }
         ...
         * @param array|\pachno\core\entities\User $users
         */
         ...
         *
         * @param \pachno\core\entities\Article $article
         * @param array|\pachno\core\entities\User $users
         */
        ...
        protected function _upgrade()
        {
            ...
            {
                case '1.0':
                    IncomingEmailAccount::getB2DBTable()->upgrade(\pachno\core\modules\mailing\upgrade_32\PachnoIncomingEmailAccountTable::getTable());
