<?php
    ...
    use pachno\core\framework,
        pachno\core\helpers\TextParser,
    ...
    class Publish extends CoreModule
    {
        ...
        protected function _addListeners()
        {
            ...
            framework\Event::listen('core', 'pachno\core\entities\Project::_postSave', array($this, 'listen_createNewProject'));
            framework\Event::listen('core', 'pachno\core\entities\File::hasAccess', array($this, 'listen_fileHasAccess'));
            framework\Event::listen('core', 'pachno\core\entities\User::__getStarredArticles', array($this, 'User__getStarredArticles'));
            framework\Event::listen('core', 'pachno\core\entities\User::__isArticleStarred', array($this, 'User__isArticleStarred'));
            framework\Event::listen('core', 'pachno\core\entities\User::__addStarredArticle', array($this, 'User__addStarredArticle'));
            framework\Event::listen('core', 'pachno\core\entities\User::__removeStarredArticle', array($this, 'User__removeStarredArticle'));
            framework\Event::listen('core', 'pachno\core\entities\\tables\Files::getUnattachedFiles', array($this, 'Files__getUnattachedFiles'));
...
//            TextParser::addRegex('/(?<!")\![A-Z]+[a-z]+[A-Z][A-Za-z]*\b/', array($this, 'stripExclamationMark'));
        ...
        public function loadFixturesArticles($scope, $overwrite = true)
        {
            if (framework\Context::isCLI())
                \pachno\core\framework\cli\Command::cli_echo("Loading default articles\n");
            $this->loadArticles('', $overwrite, $scope);
            if (framework\Context::isCLI())
                \pachno\core\framework\cli\Command::cli_echo("... done\n");
        }
            ...
            while ($original_article_name = readdir($_path_handle))
            {
                if (mb_strpos($original_article_name, '.') === false)
                {
                    ...
                    if ($import)
                    {
                        if (framework\Context::isCLI())
                        {
                            \pachno\core\framework\cli\Command::cli_echo('Saving ' . urldecode($original_article_name) . "\n");
                        }
                        if ($overwrite)
                        {
        ...
        public function postConfigSettings(\pachno\core\framework\Request $request)
        {
         ...
         * @param \pachno\core\helpers\TextParser $parser Parser used for processing the originating article.
         *
        ...
        {
            $article_ids = \pachno\core\entities\tables\ArticleFiles::getTable()->getArticlesByFileID($event->getSubject()->getID());
