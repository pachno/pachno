...
<div class="main_area article edit at-<?php echo ($article->getArticleType() == \pachno\core\entities\Article::TYPE_WIKI) ? 'wiki' : 'manual'; ?>" id="article-editor-main-container">
    <a name="top"></a>
    <?php if (isset($error)): ?>
    ...
    <form accept-charset="<?php echo \pachno\core\framework\Context::getI18n()->getCharset(); ?>" action="<?php echo make_url($article_route, $article_route_params); ?>" method="post" id="edit_article_form">
        ...
        <input type="hidden" name="article_id" value="<?php echo ($article instanceof \pachno\core\entities\Article) ? $article->getID() : 0; ?>">
        <input type="hidden" name="last_modified" value="<?php echo ($article instanceof \pachno\core\entities\Article) ? $article->getPostedDate() : 0; ?>">
        <div class="editor_header">
            <table style="border: 0;" class="padded_table" cellpadding=0 cellspacing=0>
                    ...
                    <td style="position: relative;">
                        ...
                        <?php if (!$article->getParentArticle() instanceof \pachno\core\entities\Article): ?>
                            <ul class="article-type-selector" id="article-type-selector">
                                ...
                                <li data-article-type="<?php echo \pachno\core\entities\Article::TYPE_MANUAL; ?>" data-class-name="at-manual" class="article-type <?php if ($article->getArticleType() == \pachno\core\entities\Article::TYPE_MANUAL) echo 'selected'; ?>">
                ...
                <tbody id="article_parent_container" style="<?php if ($article->getArticleType() != \pachno\core\entities\Article::TYPE_MANUAL) echo 'display: none;'; ?>">
                    <?php /*<tr>
        ...
        <div class="publish_article_actions">
            <?php if ($article->getID()): ?>
                <?php echo link_tag((($article instanceof \pachno\core\entities\Article) ? make_url('publish_article', array('article_name' => $article_name)) : make_url('publish')), __('Cancel')); ?>
            <?php endif; ?>
            <input class="button" type="submit" onclick="$('article_preview').value = 1;" value="<?php echo ($article instanceof \pachno\core\entities\Article) ? __('Preview changes') : __('Preview article'); ?>">
            <input class="button button-green" id="save_button" type="submit" value="<?php echo ($article instanceof \pachno\core\entities\Article) ? __('Save changes') : __('Create article'); ?>">
        </div>
    </form>
    <form id="parent_selector_container" class="fullpage_backdrop" style="display: none;" onsubmit="Pachno.Main.loadParentArticles(this);return false;" action="<?php echo make_url('publish_article_parents', array('article_name' => $article->getName())); ?>">
