        ...
        <?php if ($article instanceof \pachno\core\entities\Article): ?>
                        ...
                        <tbody>
                            <?php foreach ($history as $revision => $history_item): ?>
                                <tr>
                                    ...
                                    <td style="text-align: center;"><?php echo \pachno\core\framework\Context::getI18n()->formatTime($history_item['updated'], 20); ?></td>
                                    <td><i><?php echo ($history_item['author'] instanceof \pachno\core\entities\User) ? '<a href="javascript:void(0);" onclick="Pachno.Main.Helpers.Backdrop.show(\'' . make_url('get_partial_for_backdrop', array('key' => 'usercard', 'user_id' => $history_item['author']->getID())) . '\');">' . $history_item['author']->getName() . '</a>' : __('Initial import'); ?></i></td>
            ...
            <?php elseif ($history_action == 'diff'): ?>
                <p style="padding: 0 5px 10px 10px; font-size: 13px;">
                    <?php echo '<b>'.__('Showing the difference between revisions: %from_revision &rArr; %to_revision', array('&rArr;' => '<b>&rarr;</b>', '%from_revision' => '</b><i>'.__('%revision_number, by %author [%date]', array('%revision_number' => link_tag(make_url('publish_article_revision', array('article_name' => $article->getName(), 'revision' => $from_revision)), $from_revision, array('style' => 'font-weight: bold;')), '%author' => $from_revision_author, '%date' => \pachno\core\framework\Context::getI18n()->formatTime($from_revision_date, 20))).'</i>', '%to_revision' => '<i>'.__('%revision_number, by %author [%date]', array('%revision_number' => (($to_revision < $revision_count) ? link_tag(make_url('publish_article_revision', array('article_name' => $article->getName(), 'revision' => $to_revision)), $to_revision, array('style' => 'font-weight: bold;')) : $to_revision)."/{$revision_count}</b>", '%author' => $to_revision_author, '%date' => \pachno\core\framework\Context::getI18n()->formatTime($to_revision_date, 20))).'</i>'), true); ?><br />
