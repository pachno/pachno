        ...
        <div class="project_right branch_<?php echo $branch->getName(); ?>" id="commit_<?php echo $commit->getID(); ?>">
            ...
            <?php elseif (!$commit->isImported()): ?>
                <div class="message-box type-warning">
                    ...
                    <span class="actions">
                        <?php if (isset($branch)): ?>
                            <a class="button" href="<?= make_url('livelink_project_commit_import', ['project_key' => $selected_project->getKey(), 'commit_hash' => $commit->getRevision(), 'branch' => $branch->getName()]); ?>"><?= __('Update commit'); ?></a>
                        <?php else: ?>
                            <a class="button" href="<?= make_url('livelink_project_commit_import', ['project_key' => $selected_project->getKey(), 'commit_hash' => $commit->getRevision()]); ?>"><?= __('Update commit'); ?></a>
                        <?php endif; ?>
                    </span>
                </div>
            <?php endif; ?>
            <div class="commit-message">
                ...
                <?php if ($commit->getMessage()): ?>
                    <div class="overflow"><?= \pachno\core\helpers\TextParser::parseText($commit->getMessage(), false, null, [], \pachno\core\framework\Settings::SYNTAX_MD); ?></div>
                <?php endif; ?>
            </div>
            <?php if ($commit->isImported()): ?>
