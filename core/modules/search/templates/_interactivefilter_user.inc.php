<div class="filter interactive_dropdown" id="interactive_filter_<?php echo $filter->getFilterKey(); ?>" data-filterkey="<?php echo $filter->getFilterKey(); ?>" data-value="<?php echo $filter->getValue(); ?>" data-all-value="<?php echo __('Anyone'); ?>">
    ...
    <div class="interactive_menu">
        ...
        <input type="search" data-callback-url="<?php echo make_url('search_filter_findusers', array('filterkey' => $filter->getFilterKey())); ?>" placeholder="<?php echo __('Search for a user'); ?>"><?php echo image_tag('spinning_16.gif', array('class' => 'filter_indicator')); ?>
            ...
            <ul class="interactive_menu_values filter_existing_values">
                <?php foreach ($filter->getAvailableValues() as $user): ?>
                    <li data-value="<?php echo $user->getID(); ?>" class="filtervalue<?php if ($filter->hasValue($user->getID())) echo ' selected'; ?>">
                        ...
                        <input type="checkbox" value="<?php echo $user->getID(); ?>" name="filters_<?php echo $filter->getFilterKey(); ?>_value_<?php echo $user->getID(); ?>" data-text="<?php echo ($user->getID() == $pachno_user->getID()) ? __('Yourself') : $user->getNameWithUsername(); ?>" id="filters_<?php echo $filter->getFilterKey(); ?>_value_<?php echo $user->getID(); ?>" <?php if ($filter->hasValue($user->getID())) echo 'checked'; ?>>
                        <label for="filters_<?php echo $filter->getFilterKey(); ?>_value_<?php echo $user->getID(); ?>"><?php echo ($user->getID() == $pachno_user->getID()) ? __('Yourself').'<span class="hidden">'.$pachno_user->getNameWithUsername().'</span>' : $user->getNameWithUsername(); ?></label>
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
