...
<li id="filter_<?php echo $key; ?>">
    <?php if (in_array($filter, \pachno\core\entities\SearchFilter::getValidSearchFilters())): ?>
        <?php if ($filter == 'project_id' && !\pachno\core\framework\Context::isProjectContext()): ?>
            <label<?php if (!\pachno\core\framework\Context::isProjectContext()): ?> for="filter_project_id_<?php echo $key; ?>"<?php endif; ?>><?php echo __('Project'); ?></label>
            ...
            <?php endif; ?>
        <?php elseif ($filter == 'subprojects' && \pachno\core\framework\Context::isProjectContext()): ?>
            <label<?php if (!\pachno\core\framework\Context::isProjectContext()): ?> for="filter_subprojects_<?php echo $key; ?>"<?php endif; ?>><?php echo __('Include subprojects'); ?></label>
        ...
        <?php elseif (in_array($filter, array('posted', 'last_updated', 'time_spent'))): ?>
            ...
            <select id="filter_<?php echo $filter; ?>_<?php echo $key; ?>_day" onchange="Pachno.Search.Filter.setTimestamp('<?php echo $filter; ?>', '<?php echo $key; ?>');">
                <?php for($cc = 1; $cc <= 31; $cc++): ?>
                <option value="<?php echo $cc; ?>"<?php if ($cc == date('d', $selected_value)): ?> selected<?php endif; ?>><?php echo $cc; ?></option>
                <?php endfor; ?>
            </select>
            <select id="filter_<?php echo $filter; ?>_<?php echo $key; ?>_month" onchange="Pachno.Search.Filter.setTimestamp('<?php echo $filter; ?>', '<?php echo $key; ?>');">
            ...
            <select id="filter_<?php echo $filter; ?>_<?php echo $key; ?>_year" onchange="Pachno.Search.Filter.setTimestamp('<?php echo $filter; ?>', '<?php echo $key; ?>');">
        ...
        <?php elseif (in_array($filter, array('assignee_user', 'posted_by', 'owner_user'))): ?>
            ...
            <?php include_component('main/identifiableselector', array(    'html_id'             => 'filter_'.$filter.'_'.$key.'_popup',
                                                                        ...
                                                                        'callback'             => "Pachno.Search.Filter.setIdentifiable('" . make_url('get_temp_identifiable') . "', '".$filter."', '".$key."', %identifiable_value, 'user');",
        ...
        <?php elseif (in_array($filter, array('assignee_team', 'owner_team'))): ?>
            ...
            <?php include_component('main/identifiableselector', array(    'html_id'             => 'filter_'.$filter.'_'.$key.'_popup',
                                                                        'header'             => __('Please select'),
                                                                        'team_callback'         => "Pachno.Search.Filter.setIdentifiable('" . make_url('get_temp_identifiable') . "', '".$filter."', '".$key."', %identifiable_value, 'team');",
        ...
        <?php elseif (in_array($filter, array_keys($filters))): ?>
            ...
            <select name="filters[<?php echo $filter; ?>][<?php echo $key; ?>][value]" id="filter_<?php echo $filter; ?>_<?php echo $key; ?>">
                ...
                <?php foreach ($filters[$filter]['options'] as $item): ?>
                    <?php if (method_exists($item, 'getProject') && ((\pachno\core\framework\Context::isProjectContext() && $item->getProject()->getID() != \pachno\core\framework\Context::getCurrentProject()->getID()) || !\pachno\core\framework\Context::isProjectContext())): ?>
        ...
        <?php elseif ($filter == 'state'): ?>
            ...
            <select name="filters[state][<?php echo $key; ?>][value]" id="filter_state_<?php echo $key; ?>">
                <option value="<?php echo \pachno\core\entities\Issue::STATE_OPEN; ?>"<?php if ($selected_value == \pachno\core\entities\Issue::STATE_OPEN): ?> selected<?php endif; ?>><?php echo __('Open'); ?></option>
                <option value="<?php echo \pachno\core\entities\Issue::STATE_CLOSED; ?>"<?php if ($selected_value == \pachno\core\entities\Issue::STATE_CLOSED): ?> selected<?php endif; ?>><?php echo __('Closed'); ?></option>
            </select>
            <?php $show_button = true; ?>
        <?php endif; ?>
    <?php else: ?>
        <?php $customdatatype = \pachno\core\entities\CustomDatatype::getByKey($filter); ?>
    ...
    <?php endif; ?>
    <?php if ($show_button): ?>
        <button onclick="Pachno.Search.Filter.remove(<?php echo $key; ?>);"><?php echo __('Remove'); ?></button>
    <?php endif; ?>
</li>
