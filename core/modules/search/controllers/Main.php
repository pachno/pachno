...
class Main extends framework\Action
{
        ...
        if ($groupby != '')
        {
            switch ($groupby)
            {
                ...
                case 'posted':
                    ...
                    $groupby_description = \pachno\core\framework\Context::getI18n()->formatTime($issue->getPosted(), 20);
                    break;
                case 'time_spent':
                    if ($issue->getSpentTimes())
                    {
                        ...
                        $groupby_description = \pachno\core\framework\Context::getI18n()->formatTime($issue_spent_time->getEditedAt(), 20);
                    }
                    else
                    {
                        $groupby_id = 0;
    ...
    public function runBulkUpdateIssues(framework\Request $request)
    {
        ...
        $options['last_updated'] = \pachno\core\framework\Context::getI18n()->formatTime(time(), 20);
