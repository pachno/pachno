...
<table style="width: 100%;" cellpadding="0" cellspacing="0" class="search_results resizable sortable">
    ...
    <?php foreach ($search_object->getIssues() as $issue): ?>
        <tr class="<?php if ($issue->isBlocking()): ?> blocking<?php endif; ?><?php if ($issue->getUserPain() <= $search_object->getTemplateParameter() && $first): $first = false; ?> userpain_below_threshold<?php endif; ?>">
            ...
            <td class="result_issue"<?php if (\pachno\core\framework\Context::isProjectContext()): ?> style="padding-left: 3px;"<?php endif; ?>>
                <?php echo link_tag($issue->getUrl(), '<span class="issue_no">' . $issue->getFormattedIssueNo(true) . '</span> - <span class="issue_title">' . $issue->getTitle() . '</span>', array('class' => 'issue_link')); ?>
            </td>
            <td<?php if (!$issue->getStatus() instanceof \pachno\core\entities\Datatype): ?> class="faded_out"<?php endif; ?>>
                <?php if ($issue->getStatus() instanceof \pachno\core\entities\Datatype): ?>
                    <table style="table-layout: auto; width: auto;" cellpadding=0 cellspacing=0 class="status_table">
                        <tr>
                            <td style="width: 24px;"><div style="border: 1px solid #AAA; background-color: <?php echo ($issue->getStatus() instanceof \pachno\core\entities\Datatype) ? $issue->getStatus()->getColor() : '#FFF'; ?>; font-size: 1px; width: 20px; height: 15px; margin-right: 2px;">&nbsp;</div></td>
