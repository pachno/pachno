...
<li class="add_epic_container" id="add_epic_container">
        ...
        <span class="plus" onclick="$(this).up('li').toggleClass('selected');$('new_epic_title').focus();">+</span>
        <form action="<?php echo make_url('agile_addepic', array('project_key' => $board->getProject()->getKey(), 'board_id' => $board->getID())); ?>" method="post" onsubmit="Pachno.Project.Planning.addEpic(this);return false;">
                ...
                <div class="actionbuttons">
                        <?php echo __('%cancel or %add_epic', array('%cancel' => javascript_link_tag(__('cancel'), array('onclick' => "$(this).up('li').toggleClass('selected');")), '%add_epic' => '<input type="submit" class="button" value="'.__('Add epic').'">')); ?>
                </div>
        </form>
</li>
