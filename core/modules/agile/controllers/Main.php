    ...
    class Main extends helpers\ProjectActions
    {
        ...
        public function runMarkMilestoneFinished(framework\Request $request)
        {
            try
            {
                ...
                $board = \pachno\core\entities\tables\AgileBoards::getTable()->selectById($request['board_id']);
                $milestone = \pachno\core\entities\Milestone::getB2DBTable()->selectById($request['milestone_id']);
                ...
                if ($request->hasParameter('unresolved_issues_action'))
                {
                    ...
                    {
                        case 'backlog':
                            \pachno\core\entities\tables\Issues::getTable()->reAssignIssuesByMilestoneIds($milestone->getID(), null, 0);
                            break;
                        case 'reassign':
                            $new_milestone = \pachno\core\entities\Milestone::getB2DBTable()->selectById($request['assign_issues_milestone_id']);
                            if ($request['assign_issues_milestone_id'] === '' || !$new_milestone instanceof \pachno\core\entities\Milestone || $new_milestone->isClosed())
                            {
                                switch ($board->getType())
                                {
                                    case \pachno\core\entities\AgileBoard::TYPE_GENERIC:
                                        throw new \Exception($this->getI18n()->__('You must select an existing, unfinished milestone'));
                                        break;
                                    case \pachno\core\entities\AgileBoard::TYPE_SCRUM:
                                    case \pachno\core\entities\AgileBoard::TYPE_KANBAN:
                    ...
                    if (isset($new_milestone) && $new_milestone instanceof \pachno\core\entities\Milestone)
                    {
                        \pachno\core\entities\tables\Issues::getTable()->reAssignIssuesByMilestoneIds($milestone->getID(), $new_milestone->getID());
                    }
                }
        ...
        {
            $boards = \pachno\core\entities\tables\AgileBoards::getTable()->getAvailableProjectBoards($this->getUser()->getID(), $this->selected_project->getID());
        ...
        public function runBoard(framework\Request $request)
        {
            ...
            $this->board = ($request['board_id']) ? \pachno\core\entities\tables\AgileBoards::getTable()->selectById($request['board_id']) : new \pachno\core\entities\AgileBoard();

            if (!$this->board instanceof \pachno\core\entities\AgileBoard) {
        ...
        {
            $board = \pachno\core\entities\tables\AgileBoards::getTable()->selectById($request['board_id']);
            $column = \pachno\core\entities\BoardColumn::getB2DBTable()->selectById($request['column_id']);
            if (!$column instanceof \pachno\core\entities\BoardColumn)
            {
        ...
        public function runWhiteboardIssues(framework\Request $request)
        {
            ...
            $this->board = \pachno\core\entities\tables\AgileBoards::getTable()->selectById($request['board_id']);

            $this->forward403unless($this->board instanceof \pachno\core\entities\AgileBoard);

            try
            {
                if ($request->isPost())
                {
                    $issue = \pachno\core\entities\tables\Issues::getTable()->selectById((int) $request['issue_id']);
                    $column = \pachno\core\entities\BoardColumn::getB2DBTable()->selectById((int) $request['column_id']);
                    $milestone = \pachno\core\entities\Milestone::getB2DBTable()->selectById((int) $request['milestone_id']);
                    ...
                    {
                        $transitions = array(\pachno\core\entities\tables\WorkflowTransitions::getTable()->selectById((int) $request['transition_id']));
                ...
                if ($request->isPost())
                {
                    ...
                    if (is_array($columns))
                    {
                        foreach ($columns as $details)
                        {
                            ...
                            {
                                $column = \pachno\core\entities\BoardColumn::getB2DBTable()->selectById($details['column_id']);
                            }
                            else
                            {
                                $column = new \pachno\core\entities\BoardColumn();
        ...
        public function runRetrieveIssue(framework\Request $request)
        {
            $this->forward403unless($this->_checkProjectPageAccess('project_planning'));
            $board = \pachno\core\entities\tables\AgileBoards::getTable()->selectById($request['board_id']);
            $issue = \pachno\core\entities\Issue::getB2DBTable()->selectById($request['issue_id']);

            if ($issue instanceof \pachno\core\entities\Issue && !$issue->hasAccess()) return $this->renderJSON(array('child_issue' => 0, 'issue_details' => array(), 'deleted' => 1));
            ...
            if ($request['mode'] == 'whiteboard')
            {
                ...
                {
                    if ($issue->getMilestone() instanceof \pachno\core\entities\Milestone && $issue->getMilestone()->getID() == $request['milestone_id'])
                    {
                        foreach ($board->getMilestoneSwimlanes($issue->getMilestone()) as $swimlane)
                        {
                            if ($swimlane->getBoard()->usesSwimlanes()
                                ...
                                && $swimlane->getBoard()->getSwimlaneType() == \pachno\core\entities\AgileBoard::SWIMLANES_ISSUES
            ...
            else
            {
                ...
                $text['milestone_percent_complete'] = $issue->getMilestone() instanceof \pachno\core\entities\Milestone ? $issue->getMilestone()->getPercentComplete() : 0;
        ...
        public function runAddEpic(framework\Request $request)
        {
            ...
            try
            {
                ...
                $issue = new \pachno\core\entities\Issue();
        ...
        public function runMilestoneIssues(framework\Request $request)
        {
            try
            {
                ...
                {
                    case $request->isPost():
                        $issue_table = \pachno\core\entities\tables\Issues::getTable();
         ...
         * @return null|\pachno\core\entities\Milestone
         * @throws \Exception
         */
        protected function _saveMilestoneDetails(framework\Request $request, $milestone = null)
        {
            ...
            if ($milestone === null) $milestone = new \pachno\core\entities\Milestone();
            ...
            $milestone->setType($request->getParameter('milestone_type', \pachno\core\entities\Milestone::TYPE_REGULAR));
            $milestone->setPercentageType($request->getParameter('percentage_type', \pachno\core\entities\Milestone::PERCENTAGE_TYPE_REGULAR));
        ...
        public function runAssignMilestone(framework\Request $request)
        {
            ...
            try
            {
                $issue = \pachno\core\entities\Issue::getB2DBTable()->selectById((int) $request['issue_id']);
                $milestone = \pachno\core\entities\tables\Milestones::getTable()->selectById($request['milestone_id']);
                ...
                $new_issues = ($milestone instanceof \pachno\core\entities\Milestone) ? $milestone->countIssues() : 0;
                $new_e_points = ($milestone instanceof \pachno\core\entities\Milestone) ? $milestone->getPointsEstimated() : 0;
                $new_e_hours = ($milestone instanceof \pachno\core\entities\Milestone) ? $milestone->getHoursAndMinutesEstimated(true, true) : 0;
        ...
        public function runAssignRelease(framework\Request $request)
        {
            ...
            {
                $issue = \pachno\core\entities\Issue::getB2DBTable()->selectById((int) $request['issue_id']);
                $release = \pachno\core\entities\tables\Builds::getTable()->selectById((int) $request['release_id']);
        ...
        public function runAssignEpic(framework\Request $request)
        {
            try
            {
                $epic = \pachno\core\entities\Issue::getB2DBTable()->selectById((int) $request['epic_id']);
                $issue = \pachno\core\entities\Issue::getB2DBTable()->selectById((int) $request['issue_id']);

                $epic->addChildIssue($issue, true);

                return $this->renderJSON(array('issue_id' => $issue->getID(), 'epic_id' => $epic->getID(), 'closed_pct' => $epic->getEstimatedPercentCompleted(), 'num_child_issues' => $epic->countChildIssues(), 'estimate' => \pachno\core\entities\Issue::getFormattedTime($epic->getEstimatedTime(true, true)), 'text_color' => $epic->getAgileTextColor()));
            }
            catch (\Exception $e)
            {
                $this->getResponse()->setHttpStatus(400);
        ...
        public function runMilestone(framework\Request $request)
        {
            ...
            $milestone = new \pachno\core\entities\Milestone($milestone_id);
            ...
            try
            {
                ...
                switch (true)
                {
                    case $request->isDelete():
                        ...
                        $no_milestone = new \pachno\core\entities\Milestone(0);
                    ...
                    case $request->isPost():
                        ...
                        if ($request->hasParameter('issues') && $request['include_selected_issues'])
                            \pachno\core\entities\tables\Issues::getTable()->assignMilestoneIDbyIssueIDs($milestone->getID(), $request['issues']);
        ...
        public function runPoll(framework\Request $request)
        {
            ...
            if ($search_object instanceof \pachno\core\entities\SavedSearch)
            {
                $search_object->setFilter('last_updated', \pachno\core\entities\SearchFilter::createFilter('last_updated', array('o' => \b2db\Criterion::GREATER_THAN_EQUAL, 'v' => $last_refreshed - 2)));
            }

            if ($request['mode'] == 'whiteboard')
            {
                $milestone_id = $request['milestone_id'];
                $ids = \pachno\core\entities\tables\Issues::getTable()->getUpdatedIssueIDsByTimestampAndProjectIDAndMilestoneID($last_refreshed - 2, $this->selected_project->getID(), $milestone_id);
            }
            else
            {
                $ids = \pachno\core\entities\tables\Issues::getTable()->getUpdatedIssueIDsByTimestampAndProjectIDAndIssuetypeID($last_refreshed - 2, $this->selected_project->getID());
                $epic_ids = ($board->getEpicIssuetypeID()) ? \pachno\core\entities\tables\Issues::getTable()->getUpdatedIssueIDsByTimestampAndProjectIDAndIssuetypeID($last_refreshed - 2, $this->selected_project->getID(), $board->getEpicIssuetypeID()) : array();
            }
