<?php foreach ($board->getReleases() as $release): ?>
    <li class="release" id="release_<?php echo $release->getId(); ?>" data-release-id="<?php echo $release->getID(); ?>" data-assign-issue-url="<?php echo make_url('agile_assignrelease', array('project_key' => $board->getProject()->getKey(), 'release_id' => $release->getID())); ?>">
        ...
        <dl class="info">
            ...
            <dd><?php echo $release->isReleased() ? ($build->hasReleaseDate() ? \pachno\core\framework\Context::getI18n()->formatTime($release->getReleaseDate(), 20) : __('Released, without date')) : __('Not released yet'); ?></dd>
            <?php if (! $release->isReleased()): ?>
                <dt><?php echo __('Scheduled for'); ?></dt>
                <dd><?php echo $release->hasReleaseDate() ? \pachno\core\framework\Context::getI18n()->formatTime($release->getReleaseDate(), 20) : __('Not scheduled'); ?></dd>
            <?php endif; ?>
