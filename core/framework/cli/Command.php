<?php
    ...
    abstract class Command
    {
         ...
         * 
         * @return \pachno\core\entities\Module 
         */
        ...
        final protected function _processArguments()
        {
            ...
            {
                $scope = \pachno\core\entities\tables\Scopes::getTable()->selectById(self::$_named_arguments['scope']);
        ...
        public static function cliError($title, $exception, $include_sql_queries = false)
        {
            ...
            {
                if ($exception instanceof \pachno\core\framework\exceptions\ActionNotFoundException)
                {
                    self::cli_echo("Could not find the specified action\n", 'white', 'bold');
                }
                elseif ($exception instanceof \pachno\core\framework\exceptions\TemplateNotFoundException)
                {
            ...
            foreach ($trace_elements as $trace_element)
            {
                ...
                {
                    if (array_key_exists('class', $trace_element) && $trace_element['class'] == 'pachno\core\framework\Context' && array_key_exists('function', $trace_element) && in_array($trace_element['function'], array('errorHandler', 'cliError')))
                        continue;
