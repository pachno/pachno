        ...
        protected function migrateData(\b2db\Table $old_table)
        {
            switch ($old_table->getVersion())
            {
                case 1:
                    ...
                    {
                        $customfields = \pachno\core\entities\CustomDatatype::getB2DBTable()->selectAll();
                        while ($row = $res->getNextRow())
                        {
                            ...
                            if ($customfield instanceof \pachno\core\entities\CustomDatatype && $customfield->hasCustomOptions())
                            {
                                $customfieldoption = CustomFieldOptions::getTable()->getByValueAndCustomfieldID((int) $row->get(self::OPTION_VALUE), $customfield->getID());
                                if ($customfieldoption instanceof \pachno\core\entities\CustomDatatypeOption)
                                {
                                    $update = new Update();
