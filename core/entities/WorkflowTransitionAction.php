    ...
    class WorkflowTransitionAction extends IdentifiableScoped
    {
        ...
        protected static function _populateAvailableActions()
        {
            if (self::$_available_actions === null)
            {
                ...
                $event = new \pachno\core\framework\Event('core', 'WorkflowTransitionAction::getAvailableTransitionActions', null, array(), $initial_list);
                $event->trigger();
            ...
            switch ($this->_action_type)
            {
                ...
                case self::ACTION_ASSIGN_ISSUE:
                    if ($this->getTargetValue())
                    {
                        ...
                        {
                            $assignee = \pachno\core\entities\User::getB2DBTable()->selectById((int) $target_details[1]);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            case 'user':
                                $assignee = \pachno\core\entities\User::getB2DBTable()->selectById((int) $request['assignee_id']);
                                break;
                            case 'team':
                                $assignee = Team::getB2DBTable()->selectById((int) $request['assignee_id']);
                                break;
                        }
                        if ((bool) $request->getParameter('assignee_teamup', false) && $assignee instanceof \pachno\core\entities\User && $assignee->getID() != framework\Context::getUser()->getID())
                        {
                            $team = new \pachno\core\entities\Team();
                ...
                case self::ACTION_USER_START_WORKING:
                    ...
                    if ($issue->getAssignee() instanceof \pachno\core\entities\Team && $issue->getAssignee()->isOndemand())
                    {
                        $members = $issue->getAssignee()->getMembers();
                        $issue->startWorkingOnIssue(array_shift($members));
                    }
                    elseif ($issue->getAssignee() instanceof \pachno\core\entities\User)
                    {
                ...
                case self::ACTION_USER_STOP_WORKING:
                    ...
                    elseif ($request->getParameter('did', 'nothing') == 'this')
                    {
                        ...
                        {
                            $times['hours'] *= 100;
                            $spenttime = new \pachno\core\entities\IssueSpentTime();
                ...
                default:
                    ...
                    {
                        $event = new \pachno\core\framework\Event('core', 'WorkflowTransitionAction::perform', $issue, array('request' => $request));
        ...
        public function hasValidTarget()
        {
            ...
            switch ($this->_action_type)
            {
                case self::ACTION_ASSIGN_ISSUE:
                    $target_details = explode('_', $this->_target_value);
                    return (bool) ($target_details[0] == 'user') ? \pachno\core\entities\User::doesIDExist($target_details[1]) : Team::doesIDExist($target_details[1]);
                    break;
                case self::ACTION_SET_PERCENT:
        ...
        public function isValid(\pachno\core\framework\Request $request)
        {
            if ($this->_target_value) return true;
