...
import covertor from "./markdown";

let Pen, debugMode, selection = {};
...
let lineBreakReg = /^(?:blockquote|pre|div)$/i;

let effectNodeReg = /(?:[pubia]|h[1-6]|blockquote|[uo]l|li)/i;
...
function commandOverall(ctx, cmd, val) {
    let message = ' to exec 「' + cmd + '」 command' + (val ? (' with value: ' + val) : '');
...
function commandBlock(ctx, name) {
    let list = effectNode(ctx, getNode(ctx), true);
...
function initToolbar(ctx) {
    let icons = '', inputStr = '<input class="pen-input" placeholder="http://" />';

    ctx._toolbar = ctx.config.toolbar;
    if (!ctx._toolbar) {
        let toolList = ctx.config.list;
        utils.forEach(toolList, function (name) {
            let klass = 'pen-icon icon-' + name;
            let title = ctx.config.titles[name] || '';
...
function initEvents(ctx) {
    let toolbar = ctx._toolbar || ctx._menu, editor = ctx.config.editor;

    let toggleMenu = utils.delayExec(function() {
        ctx.highlight().menu();
    });
    ...
    let menuApply = function(action, value) {
...
function removeListener(ctx, target, type, listener) {
    ...
    if (!events) {
        let _index = ctx._eventTargets.indexOf(target);
...
function removeAllListeners(ctx) {
    ...
    utils.forEach(ctx._eventsCache, function (events, index) {
        let target = ctx._eventTargets[index];
...
function getNode(ctx, byRoot) {
    let node, root = ctx.config.editor;
...
function lineBreak(ctx, empty) {
    let range = ctx._range = ctx.getRange(), node = document.createElement('p');
...
function autoLink(node) {
    if (node.nodeType === 1) {
        ...
        let result = urlToLink(node.nodeValue || '');
...
function urlToLink(str) {
    ...
    str = str.replace(autoLinkReg.url, function(url) {
        let realUrl = url, displayUrl = url;
        count++;
...
Pen = function(config) {
    ...
    // merge user config
    let defaults = utils.merge(config);
...
Pen.prototype.checkContentChange = function () {
    let prevContent = this._prevContent, currentContent = this.getContent();
...
Pen.prototype.getRange = function() {
    let editor = this.config.editor, range = selection.rangeCount && selection.getRangeAt(0);
...
Pen.prototype.highlight = function() {
    let toolbar = this._toolbar || this._menu
        , node = getNode(this);
    ...
    let effects = effectNode(this, node)
    ...
    highlight = function(str) {
        if (!str) return;
        let el = toolbar.querySelector('[data-action=' + str + ']');
...
Pen.prototype.menu = function() {
    ...
    let offset = this._range.getBoundingClientRect()
        , menuPadding = 10
...
Pen.prototype.destroy = function(isAJoke) {
    let destroy = isAJoke ? false : true
