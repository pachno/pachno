...
"""
Hook to submit pushed commits to Pachno.
...
The hook must be configured with details of your installation of Pachno. There are
two methods for submitting commit information. If your repository is hosted on
the same server as Pachno, you should use the direct call method, which runs the
...
necessary (commands should be run in the repository directory on the server):
 ...
 $ bzr pachno-configure &lt;project ID&gt; direct &lt;full path to bin/pachno&gt; &lt;path to PHP binary&gt;
...
case, the following configuration is necessary:
 ...
 $ bzr pachno-configure &lt;project ID&gt; http &lt;report URL&gt; &lt;passkey set in module settings&gt;

The report URL being e.g. http://mydomain.com/pachno/vcs_integration/report
...
## Class to submit revision details to Pachno.
class PachnoSubmitter:
	...
	def submit_direct(self, author, rev, message, date, files, oldrev):
		# Script must be executed from the Pachno directory.
		prev_cwd = os.getcwd()
...
def _pachno_hook_change_tip(params, dry_run = False):
	...
	project = config.get_user_option('pachno_project')
	if not project:
		return
	method = config.get_user_option('pachno_method')
	opt1 = config.get_user_option('pachno_method_opt1')
	opt2 = config.get_user_option('pachno_method_opt2')

	# Create the submitter object.
	submitter = PachnoSubmitter(method, opt1, opt2, project, dry_run)
	...
	for r in revisions:
		...
		# Submit the revision.
		info("Submitting revision %s to Pachno." % (revno))
		submitter.submit(
			author, revno, revision.message, revision.timestamp,
			added, removed, changed, oldrev
		)

# Hook to submit pushed revisions to Pachno.
def pachno_hook_change_tip(params):
	_pachno_hook_change_tip(params)
Branch.hooks.install_named_hook('post_change_branch_tip', pachno_hook_change_tip, 'Pachno')

# Command to configure the Pachno hook.
@register_command
class cmd_pachno_configure(Command):
	"""Configure the Pachno hook."""
	...
	def run(self, project, method, arg1, arg2):
		...
		config.set_user_option('pachno_project', project)
		config.set_user_option('pachno_method', method)
		config.set_user_option('pachno_method_opt1', arg1)
		config.set_user_option('pachno_method_opt2', arg2)
		return 0
...
if __name__ == '__main__':
	...
	_pachno_hook_change_tip(params, True)
	sys.exit(0)