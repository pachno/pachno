<?php
    $base_url = \pachno\core\framework\Context::getModule('vcs_integration')->getSetting('browser_url_' . $projectId);
    ...
    $link_base = \pachno\core\framework\Context::getModule('vcs_integration')->getSetting('commit_url_' . $projectId);
...
<div class="comment commit <?php if ($branchname !== null): ?>branch_<?php echo $branchname; ?><?php endif; ?>" id="commit_<?php echo $commit->getID(); ?>">
    <div style="position: relative; overflow: visible; padding: 5px;" id="commit_view_<?php echo $commit->getID(); ?>" class="comment_main">
        <div id="commit_<?php echo $commit->getID(); ?>_header" class="commentheader">
            ...
            <div class="commentdate" id="commit_<?php echo $commit->getID(); ?>_date"><?php echo \pachno\core\framework\Context::getI18n()->formatTime($commit->getDate(), 12); ?> - <?php echo __('Preceeded by %prev', array('%prev' => '<a href="'.$link_old.'" target="_blank">'.$commit->getPreviousRevision().'</a>'))?></div>
        </div>
            ...
            <div class="commit_expander" style="<?php if (isset($expanded) && $expanded == true) echo 'display: none;'; ?>">
                ...
                <div id="checkin_details_<?php echo $commit->getID(); ?>" style="<?php if (!isset($expanded) || $expanded == false) echo 'display: none;'; ?>">
                            ...
                            <table border=0 cellpadding=0 cellspacing=0 style="width: 100%;">
                                ...
                                else
                                {
                                    foreach ($commit->getFiles() as $file)
                                    {
                                        ...
                                        $link_file = str_replace('%revno', $revision, \pachno\core\framework\Context::getModule('vcs_integration')->getSetting('log_url_' . $projectId));
                                        ...
                                        $link_diff = str_replace('%revno', $revision, \pachno\core\framework\Context::getModule('vcs_integration')->getSetting('diff_url_' . $projectId));
                                        ...
                                        $link_view = str_replace('%revno', $revision, \pachno\core\framework\Context::getModule('vcs_integration')->getSetting('blob_url_' . $projectId));
                    ...
                    <div class="commit_right">
                        ...
                        {
                            if ($issue instanceof \pachno\core\entities\Issue && $issue->hasAccess())
                            {
                                $valid_issues[] = $issue;
                            }
                        }
                        ...
                        else
                        {
                            ...
                            {
                                echo '<li>'.link_tag($issue->getUrl(), $issue->getFormattedIssueNo(true)).'</li>';
                            }
                            echo '</ul>';
                        }
                        ?>
